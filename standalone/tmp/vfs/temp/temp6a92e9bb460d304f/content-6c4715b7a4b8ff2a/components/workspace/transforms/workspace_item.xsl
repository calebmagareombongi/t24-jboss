		...
		<table>
			<tr class="fragmentContainer">
				<td>
					...
					<xsl:call-template name="fragment_cmd"></xsl:call-template>
				</td>
			</tr>
		</table>
	</xsl:template>
		...
		<iframe marginwidth="0" marginheight="0" frameborder="0">
			...
			<xsl:choose>
				...
				<xsl:when test="cmdline!=''">
					<xsl:attribute name="src">../components/workspace/jsp/cmdrequest.jsp?&amp;routineName=<xsl:value-of select="routine"/>&amp;routineArgs=<xsl:value-of select="args"/>&amp;skin=<xsl:copy-of select="$skin"/>&amp;compId=<xsl:copy-of select="$companyid"/>&amp;compScreen=<xsl:copy-of select="$compScreen"/>&amp;contextRoot=<xsl:copy-of select="$contextRoot"/>&amp;windowName=<xsl:copy-of select="n"/>&amp;user=<xsl:copy-of select="$user"/>&amp;command=<xsl:value-of select="cmdline"/>&amp;formToken=<xsl:value-of select="$csrfToken"/></xsl:attribute>
				</xsl:when>
				<xsl:when test="url!=''">
