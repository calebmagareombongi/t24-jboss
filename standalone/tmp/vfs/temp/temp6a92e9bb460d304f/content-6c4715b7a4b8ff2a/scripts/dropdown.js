...
function dropdown(_application, _textField, _fieldName, _popupDropField,selectionDisplay)
{
	...
	if(selectionDisplay)
	{
	    ...
	    doEnquiryRequest(_DROPDOWN_, _application, _textField, "", "", routineArgs, "NEW", "", _DISPLAY_);
	...
	else
	{
		if(popupDropDown_GLOBAL)
		{
			...
			doEnquiryRequest(_DROPDOWN_, _application, _textField, "", "", routineArgs, "", "", _DISPLAY_);
		}
		else
		{
...
function enquiryDropdown(thisEvent)
{
	...
	if(selectionDisplay)
	{
	    ...
	    doEnquiryRequest(_DROPDOWN_, enqName, _dropField, "", "", routineArgs, "NEW", "", _DISPLAY_);
		popupDropDown_GLOBAL = savepopupDropDown_GLOBAL;
	}
	else
	{
	...
	{
		// Display the dropdown page as a fragment
		doEnquiryRequest(_DROPDOWN_, enqName, _dropField, "", "", routineArgs, "", "", _DISPLAY_);
	}
	else
	{
...
function enquiryBuildDropdown(thisEvent)
{
	...
	if(selectionDisplay)
	{
		...
		var mycmd = _DROPDOWN_;
 		...
 		//ParentFormId is set for selectionDisplay to identify parent form in AA screen.
 		updatedFormsField("WS_parentFormId",currentForm_GLOBAL);
	...
	else
	{
		...
		{
			var appreqForm = FragmentUtil.getForm(currentForm_GLOBAL);
			var dropFieldValue = getFormFieldValue(currentForm_GLOBAL, _dropField);
			
			dropFieldValue = stripSpacesFromEnds(dropFieldValue);

			if ( ( dropFieldValue == undefined ) || ( dropFieldValue == null ) )
			{
				dropFieldValue = "";
			}
			...
			{
				// Add wildcard suffix if no dots already at the end of the string
				if ( dropFieldValue.charAt( dropFieldValue.length - 1 ) != "." )
				{
					dropFieldValue = dropFieldValue + "...";
				}

				dropFieldValue = "@ID LK " + dropFieldValue;
				appreqForm.routineArgs.value= "_" + enqName + "_" + _dropField + "_" + dropFieldValue;
			}
		...
		else
		{
			...
			if ( dropFieldValue != "" )
			{
				...
				dropFieldValue = "@ID LK " + dropFieldValue;
				routineArgs = routineArgs + dropFieldValue + "_" + transId;
			}
			enquiryWindowName = createResultWindow( mycmd, 600, 300 );
			updatedFormsField( "WS_parentWindow",window.name );
...
function enquiryBuildSelect(thisEvent)
{
	...
	if(selectionDisplay)
	{
		...
		var mycmd = _DROPDOWN_;
 		enquiryWindowName = createResultWindow( mycmd, 600, 300 );
 	    // Get current window.name to update parent window
 		updatedFormsField("WS_parentWindow",FragmentUtil.getWindowOrFragmentName());
 		//ParentFormId is set for selectionDisplay to identify parent form in AA screen.
 		updatedFormsField("WS_parentFormId",currentForm_GLOBAL); 		
...
function dropDownPick( _value , _fieldName, _enrichment, fragmentName)
{
    ...
    if (dropFieldObject != null )
    {
    	...
    	/*There is no requirement to call doFieldChangeEvent explicitly for firefox as in case of any changes in the field value,
    	doFieldChangeEvent gets triggered on tabout, code below is commented out to prevent triggering of FieldContextEnquiry
    	twice in firefox
    	if( isFirefox())
    	{
    ...
    if( dropFieldObject.getAttribute("oldValue") == dropFieldObject.value ){
		// Just return as vaule of the field is not changed.
		return;
	}
	...
	// IE behaves differently for onblur event in dropdown pickup
	// so Hot field validation codes from doFieldChangeEvent() pasted here for IE to trigger dropdown hot field immediately
	// If hot field validation executed already for an enrichmentOnly field then ignore
	if( (isInternetExplorer() || isEdge()) && hotExecute )
	{
		// If the field is a hot field then run the validation
		var hotfield = dropFieldObject.getAttribute("hot");
		var hotval = dropFieldObject.getAttribute("hotVal");
...
function pick(_v, _fieldName, _enrichment)
{
    ...
    if((top.opener != null) && !(top.opener.closed) && top.opener.window.currentFragment_GLOBAL) //Parent window is not closed and we have a fragment ID
	{
		 isFragment = true;
	}  
	...
	if (_fieldName.indexOf("value:") == 0)
	{
		...
		if(isFragment) 
		{
			formId = "enqsel"+"_"+top.opener.window.currentFragment_GLOBAL; //fragment 
	    }
	    else
	    {
	    	formId = "enqsel";
	    }
		var dealField = setFieldValue( _v, _fieldName,formId );
	}
	else
	{
		//normal contract screen or fragment
	    if(isFragment) 
		{
			formId = "appreq"+"_"+top.opener.window.currentFragment_GLOBAL;  //fragment 
	}
	else
	{
	    	formId = "appreq";
	 ...
	 if ( dealField != null )
	{
		...
		if(enriObject != null) {
			...
			{		
				...
				var hotval = dealField.getAttribute("hotVal");
		...
		// Focus on the field as in Chrome and Firefox focus is getting lost from field when dropdown list is opened.
	    dealField.focus();
	    // reset the hot/auto-launch field after value is picked, doFieldFocusEvent takes care of this.
		parentWin.resetHotField_Global = "";
		doFieldFocusEvent(dealField);
...
function setFieldValue( _value, _fieldName, _form )
{
	...
	if ( ( parentWin != undefined ) && ( parentWin != null ) )
	{
		...
		if(parentFormId!=null && parentFormId!="" && parentFormId!=undefined)
		{
			if((top.opener != null) && !(top.opener.closed) && !top.opener.window.currentFragment_GLOBAL)
			{
				form = top.opener.document.getElementById(parentFormId);
			}
			else if ((top.opener != null) && !(top.opener.closed) && top.opener.window.currentFragment_GLOBAL)
			{
				form = 	FragmentUtil.getForm(_form, parentWin);	//get the fragment.
			}
			else if ((FragmentUtil.getParentWindow()!=null) && !(FragmentUtil.getParentWindow().closed))
			{
				...
				if ((field != null) && (field.name == _fieldName))
				{
					...
					if ( (field.disabled == false  &&  field.type != "hidden") || (dropEnriObject.className != null && dropEnriObject.className != '' && dropEnriObject.className.indexOf("enrichmentonly") != -1) )
					{
						...
						if(field.type != "hidden" && field.disabled == false) {
							if(_form.substr(0,6) != "enqsel")	//form might be a fragment(for eg. enqsel_fragmentID),so use substr.
						{
...
function initCalendarPopup(dropDownId) {
	...
	scrollIntoViewIfRequired(calendarDiv);
}
...
function dropDownCalendar(thisEvent)
{
	...
	loadingIconType = "dropdown";
	if(thisField.id == "" && document.getElementById(_textField) && document.getElementById(_textField).value != ""){
		displayDatedCalendar(_textField);
	}else{
...
function enqselDropDownCalendar(dropFieldId)
{
	...
	loadingIconType = "dropdown";
	if ( document.getElementById(dropFieldId) && document.getElementById(dropFieldId).value != "" ){
		displayDatedCalendar(dropFieldId);
	}else{
...
/**
 * Displays a dropdown version of the calendar if the date field has already a value.
 ...
 */
function displayDatedCalendar(dropFieldId) {
	// Get the drop field
    ...
    genForm.routineName.value = _OS__GET__CALENDAR_;
	...
	var monthYearArgs = "";
	var drpFld = document.getElementById(dropFieldId);
	...
	if(drpFld != null && drpFld.getAttribute("dateFormat"))	{
		var dtStr = drpFld.value;
		switch(drpFld.getAttribute("dateFormat")) {
			case '1' : // Check for date format 1(DD MMM YYYY)
				if(isNaN(dtStr)){
					var userMonths = drpFld.getAttribute("monthDesc").split(" ");	//Get language specific month description 
					var mm = (userMonths.indexOf(dtStr.substr(3,3))+1).toString();
					if(mm.length==1){
						mm = "0" + mm;
					}
					monthYearArgs = mm + ":" + dtStr.substr(7,4) + ":" + dtStr.substr(0,2);	
				}
				break;
			case '2' : // Check for date format 2(DD/MM/YYYY)
				if(isNaN(dtStr)){
					monthYearArgs = dtStr.substr(3,2) + ":" + dtStr.substr(6,4) + ":" + dtStr.substr(0,2);
				}
				break;
			case '3' : // Check for date format 3(YYYY/MM/DD)
			case '5' : // Check for date format 5(MM/DD/YYYY)
				if(isNaN(dtStr)){
					dtStr = new Date(dtStr);
					var yyyy = dtStr.getFullYear().toString();
					var mm = (dtStr.getMonth()+1).toString(); // getMonth() is zero-based
					var dd  = dtStr.getDate().toString();
					if(mm.length==1){
						mm = "0" + mm;
					}
					if(dd.length==1){
					...
					var fnlStr = yyyy + mm + dd;
					monthYearArgs = fnlStr.substr(4,2) + ":" + fnlStr.substr(0,4) + ":" + fnlStr.substr(6,2);
				}
				break;
			case '6' : // Check for date format 6(DD.MM.YYYY)
				if(dtStr.indexOf(".") == -1){
					monthYearArgs = dtStr.substr(2,2) + ":" + dtStr.substr(4,4) + ":" + dtStr.substr(0,2);
				}else{
					monthYearArgs = dtStr.substr(3,2) + ":" + dtStr.substr(6,4) + ":" + dtStr.substr(0,2);
				}
		}	// Check for date format 4(YYYYMMDD) and default case
		if(monthYearArgs == ""){
			monthYearArgs = drpFld.value.substr(4,2) + ":" + drpFld.value.substr(0,4) + ":" + drpFld.value.substr(6,2);
		}
	}
...
function setPopupDropDownDimensions(dropField, w, h) {
	...
	var ddContentBox = document.getElementById(dropFieldId + ":div");
	var ddBackground = document.getElementById(dropFieldId + ":iframe");
	//to check right to left alignment
	var DirValue  = document.body.dir;
	...
	if(DirValue && DirValue.toLowerCase() == "rtl")
	{
		x=x-w;
		if (x >= 0 && !(w== -1)) {
...
function hidePopupDropDown(dropFieldId) {
	...
	if (dropFieldObjFoc != null && (dropFieldObjFoc.getAttribute("tabname") != currentTabName_GLOBAL))
    {	
		dropFieldObjFoc = getCurrentTabField(currentForm_GLOBAL, dropFieldId);
	}	
...
function setPopupDropDownFocus(blurFunc, dropField) {
	...
	if (dropDownendRow != null && dropDownendRow != "") {
		try {
			dropDownendRow.focus();
		} catch (e) {};
	}

	if (ddContentBox != null && ddContentBox != "") {
		scrollIntoViewIfRequired(ddContentBox);
	}
...
function dropCalendarPickDate(_day, _month, _year, _fieldName, fragmentName)
{
	...
	if (dropFieldObject != null )
	{
		...
		dateFormat = dropFieldObject.getAttribute("dateFormat"); // Check for date format 6.
		if(dateFormat && dateFormat == "6"){
			dropFieldObject.value =  _day + "." + _month + "." + _year;  // Update the field
		}else{
			dropFieldObject.value =  _year + _month + _day;  // Update the field
		}
	...
	if (autofield != undefined) {
		doFieldContextEnquiry(autoLaunch, _fieldName);
		// This will make sure that the autofield runs only once.
		// n.b. the autofield will be enabled again on receiving focus.
		setAutoField("false");
	}
	...
	if (hotfield != undefined) {
		if (top.noFrames) {
			dropFieldObject.blur();
		} else {
			processHotField(_fieldName, dropFieldObject);
			// This will make sure that the hotfield runs only once.
			// n.b. the hotfield will be enabled again on receiving focus.
			setHotField("false", _fieldName);
		}
	}
	...
	{
		if(dateFormat && dateFormat == "6"){ // Check for date format 6.
			setEnrichment( _fieldName, dropFieldObject.value);
		}else{
...
function calendar(_textField)
{
	...
	var popupArgs = "title='Calendar Popup' &routineArgs=" + "&routineName=" + _OS__GET__CALENDAR_ + "&searchCriteria=&dropfield=" + _textField + "&skin=" + skin + "&parentWin=" + parentWin;
...
function updateCalendar(thisEvent)
{
	...
	var popupArgs = "title='Calendar Popup' &routineArgs=" + "&routineName=" + _OS__GET__CALENDAR_ + "&searchCriteria=" + monthYearArgs + "&dropfield=" + _textField + "&skin=" + skin + "&parentWin=" + parentWin;
	loadingIconType = "dropdown";
	// Display the calendar page
	displayPopup( "Calendar", popupArgs );
	loadingIconType = "normal";
...
//If image ID field is hidden then it wont have id attribute,so get the value from hidden field object.
	if (_imageID == "")
	{
	    var FieldObject = getFormFields( currentForm_GLOBAL, "fieldName:UPLOAD.ID");
	    if (FieldObject[0].type == "hidden")
	    {
 			_imageID = FieldObject[0].value;
 		}
 	}
	...
	if (_imageID != "")
	{
		// Set-up any arguments for the calendar page
		var skin = getSkin();
		var popupArgs = "title='Attachment Popup' &routineArgs=BUILD_" + _imageID + "&routineName=" + _OS__ATTACHMENT_ + "&searchCriteria=&dropfield=" + _textField + "&skin=" + skin;
		// Add form token parameter to request.
		if(arguments.length > 1) {
			var tokenParam = arguments[1];
			if(tokenParam != "" && tokenParam != _ISSUE__TOKEN_){
				popupArgs += tokenParam;
		    }
		}
...
function fileUpload(_textField)
{
	...
	attForm.routineName.value=_OS__ATTACHMENT_;
	attForm.routineArgs.value=_UPLOAD_;
...
{
	var FileDownloadform = FragmentUtil.getForm("enquiry");
	var enqid = FileDownloadform.getElementById("enqid").value;
	...
	if (navigator.userAgent.toLowerCase().indexOf("opera") != -1)
	{
		...
		if(FileDownloadform.imageId==null || FileDownloadform.imageId==undefined )
		{
			...
			var enquiryId = document.createElement("input");
			enquiryId.type = "hidden";
			enquiryId.name = "enqid";
			enquiryId.value = enqid;
			FileDownloadform.appendChild(enquiryId);
		...
		requestUrl = requestUrl+"imageId="+fileReference+"&isPopUp=YES&enqid="+enqid;
...
function enqselDropDown(_textField, _enq, _popupDropField,_selectionDisplay) {
	...
	if(_selectionDisplay == "Yes")
	{
			...
			doEnquiryRequest(_DROPDOWN_, _enq, _textField, "", "", routineArgs, "NEW", "", _DISPLAY_);
			if (savepopupDropDown_GLOBAL)
			{
				//restore the state otherwise all future dropdowns will popup
				popupDropDown_GLOBAL = savepopupDropDown_GLOBAL;
	...
	else
	{
		if(popupDropDown_GLOBAL )
		{
			doEnquiryRequest(_DROPDOWN_, _enq, _textField, "", "", routineArgs, "", "", _DISPLAY_);
		}
		else
		{
			doEnquiryRequest(_DROPDOWN_, _enq, _textField, "", "", routineArgs, "NEW", "", _DISPLAY_);
