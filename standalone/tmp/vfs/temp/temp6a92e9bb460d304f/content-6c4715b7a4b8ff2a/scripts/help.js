...
function help(thisEvent)
{
	...
	_version = getFormFieldValue( currentForm_GLOBAL, "version");
	...
	var commandArgs = "command=view&language=" + language + "&";
	var args = "";
	
	var tokenParam = "";
	if(arguments.length > 3) {
		tokenParam = arguments[3];
	}
	...
	{	
		...
		_version = getFieldValue( "version" );
	...
	else
	{
		...
		args = commandArgs + "product=" + _product + "&application=" + _application + "&field=" + _field + "&version=" + _version;
		// Add form token parameter to request.
		if(tokenParam != "" && tokenParam != _ISSUE__TOKEN_){
			args += tokenParam;
		}
...
/**
 * Toggle the Application and Version level helptext.
 * @return {void}
 */
function toggleHelp()
{
	...
	// Set-up the arguments to the HelpText servlet	
	helpForm.requestType.value = "HELP.CREATE";
	helpForm.command.value = "toggle.view";
