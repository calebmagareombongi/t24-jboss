...
function doFieldContext(ceids,enqids, enqdescs, fname,skin,olddata, realfname,fieldnum)
{
	...
	// encodeURI() replaced by encodeURIComponent() to encode the special characters
	...
	var args = "fname=" + fname + "&skin=" + skin + "&fname=" + fname + "&actualfname=" + realfname;
	...
	var lngDir = document.body.dir;
	if( !lngDir ){
		lngDir = 'LTR';
	}
	args += "&lngDir="+lngDir;
	var noRightClick = getFormFieldValue("generalForm","noRightClick");
	if( !noRightClick ){
		noRightClick = '';
	}else
	{
	...
	args += "&noRightClick="+noRightClick;
	// Add form token parameter to request.
	if(arguments.length > 8) {
		var tokenParam = arguments[8];
	...
	/*
	 * If context enquiry is triggered from a fragment then nochangeMapId and
	 * noaddSetId will have fragment name as a window name. So, we need to pass
	 * the fragment.
	 */
	if (currentFragment_GLOBAL) {
		windowName = currentFragment_GLOBAL;
	}
	windowName += "_context_";

	var myWin=window.open(fullURL, windowName, _winDefs);
	// Need to turn this into a jsp...
	
	if (myWin) {		
		myWin.focus(); //Focus on opened context enquiry window.
	}
}
/**
 ...
 */ 
 function initDeal(windowTop, windowLeft, windowWidth, windowHeight)
 {
	...
	getAppFormsNames();
	if (!window.noFrames && FragmentUtil.isCompositeScreen()) {
		for ( var i = 0; i < window.parent.frames.length; i++ )                          
		{                                                                                
			var frame = window.parent.frames[i];
			if(frame.frameElement.name == window.name){
				if(frame.frameElement.scrolling == "no" && document.getElementById("contract_screen_div")){
					document.getElementById("contract_screen_div").style.overflow = "hidden";
				}
			}                                                              
		}
	}
	...
	if ((transId.type!="hidden") && (transId.value==""))
	{
		...
		if ( ftn == "L" )
		{
			...
			else
			{
				...
				doenqDropfieldList( _ENQ_ + " %" + app + ver, dropfield );
			}
		...
		else if ( ftn == "E" )
		{
			...
			{
				// Get enquiry results using the full selection criteria
				doenqDropfieldList( _ENQ_ + " %" + app + "$NAU" + ver , dropfield );
			}
		...
		else
		{
			...
			else
			{
				...
				appreqForm.ofsOperation.value=_PROCESS_;
				...
				else if ( ftn == "LOCK" )
				{
					appreqForm.ofsOperation.value=_BUILD_;
					appreqForm.ofsFunction.value="S";
					appreqForm.routineArgs.value=_LOCK_; // We need to lock the record - for old Server code compatibility
					appreqForm.lockArgs.value=_LOCK_;    // We need to lock the record
				}
				else if ( cmdname != "" )
				{
					// Command was set to run as a utility request
					appreqForm.ofsFunction.value="S";
					buildUtilityRequest( _OS__NEW__DEAL_, cmdname, currentForm_GLOBAL, window.name, "", "", "", "", "" );
				}
...
function doNewDeal()
{
	...
	// Set-up parameters for a Globus command and submit it
	appreqForm.requestType.value=_UTILITY__ROUTINE_;
	appreqForm.ofsOperation.value=_PROCESS_;
	appreqForm.routineName.value=_OS__NEW__DEAL_;
...
function doHotValidate(fieldName)
{
	...
	if (hotField.getAttribute("hotVal") == "Y")
	{
		...
		doDeal("VAL");
		// This will make sure that the hotfield runs only once for calendar frequency field.
		// n.b. the hotfield will be enabled again on receiving focus.
		if (hotField.getAttribute("frqfld") == "Y"){
			setHotField("false", fieldName);
		}
	}
...
function doHotField( _fieldName )
{
	...
	if(top.noFrames)
	{
		...
		//if currentFragmentObject is null then skip
  ...
  function doWebValidationField( _fieldName, _validationRoutine )
{
	...
	requestType = _WEB__VALIDATION_;			// The type of the request
	clearParams = false;
...
function doSearchListEnquiry( searchOp, enqName )
{
	...
	if ( searchOp == "SEARCH" )
	{
		...
		appreqForm.requestType.value = _UTILITY__ROUTINE_;
		appreqForm.routineName.value= _OS__ENQUIRY__SELECTION_;
		appreqForm.routineArgs.value = enqName;
	}
	else if ( searchOp == "LIST" )
	{
		// Return a list of the file - a full enquiry result
		appreqForm.requestType.value= _OFS__ENQUIRY_;
		appreqForm.operation.value= _ENQUIRY__SELECT_;
...
{	
	...
	{
		appreqForm.enqaction.value = _SELECTION_;
 ...
 function doPaste( myItem )
{
	...
	appreqForm.requestType.value = _UTILITY__ROUTINE_;
	appreqForm.ofsOperation.value = _PROCESS_;
	appreqForm.routineName.value = _OS__PASTE__DEAL_;
...
function submitPrintSlips(reportList,formName) {
	...
	form.requestType.value = _OFS__GET__DEAL__SLIP_;
...
function doPrintSlip()
{
	...
	appreqForm.requestType.value=_OFS__PRINT__DEAL__SLIP_;
...
function doCopy()
{
	...
	// Add histNo in routinArgs to help T24 to copy the history records.
	var histNo = getFieldValue("histNo");	
	if (histNo)
	{
		myDesc = myDesc + histNo;
	}
	...
	// Set-up parameters for a Globus command and submit it
	appreqForm.requestType.value=_OFS__OS__COPY_;
	appreqForm.ofsOperation.value=_POPULATE_;
	...
	/*
	 * If copy function is triggered from a fragment then nochangeMapId and
	 * noaddSetId will have fragment name as a window name. So, we need to pass
	 * the fragment.
	 */
	if (currentFragment_GLOBAL) {
		appreqForm.windowName.value = currentFragment_GLOBAL;
	}
...
function doFieldContextEnquiry(enqName, fieldName)
{
	if (autoLaunch == "Y")
	{
		...
		enquiryType = _CONTEXT__ENQUIRY_;
 ...
 */
 function appendSeeModeRequest(formName)
 {
	...
	addFormElement( formName + ":requestType", _UTILITY__ROUTINE_);
	addFormElement( formName + ":ofsFunction", BrowserFunction.See);
	addFormElement( formName + ":routineName", _OS__NEW__DEAL_);
 ...
 */
 function setActionsOnForm(formName, ofsFunction)
 {
 	// Get formName object.
  	...
  	formObj.requestType.value=_OFS__APPLICATION_;
    ...
    switch (ofsFunction)
    {
        ...
        case BrowserFunction.Input:
        	//
        	formObj.ofsOperation.value=_PROCESS_;	 
        	break;
        case BrowserFunction.Validate:
        	//
            formObj.ofsOperation.value=_VALIDATE_;
 ...
 */ 
 function doToolbar( action, ofsFunction, prompt, promptText )
 {
 	...
 	// when warnings present,change commit action to related functions(for eg. while reversing LD contract.)
 	var screenMode = getFormFieldValue(currentForm_GLOBAL, "screenMode");
 	if (screenMode != "")
 	{
 		var warningsPresent = getFormFieldValue(currentForm_GLOBAL, "warningsPresent");
 		if (warningsPresent && ofsFunction == "I" && screenMode != "I")
 		{
 			ofsFunction = screenMode ;	//actual ofsFunction
 		}
 	}
	/**
  * check whether we are going to perform delete function and also should not display the delete confirm msg.
  * warning pop up message is displayed only for Internal users.
  */ 
	var ArcUser =  window.document.getElementById("isARC");
	if( ofsFunction == "D" && delMsgDisplayed.value != 'Yes' && !( ArcUser != "" && ArcUser != null && ArcUser.value == "true" ) )
	{
	...
	//Perform the DATE.FORMAT 6 conversion for DATE fields
	if(ofsFunction == "I" || ofsFunction == "VAL")
	{
	    ...
	    for ( var i = 0; i < formsNumber; i++ )
	    {
	        var form = window.document.forms[i];
	        	    ...
	        	    for ( var j = 0; j < form.elements.length; j++ )
	        	    {
                        ...
                        var fldValue = field.value.replace(/^\s+|\s+$/g,"");

                        // Check for date format 6 and if so add dot separator while submit the transaction
                        if (dateFrmt == "6" && fldValue != "" && fldValue.indexOf(".") == -1 && fldValue.length >= 8 && !isNaN(fldValue.substr(0,8)))
                        {
                            field.value = fldValue.substr(0,2) + "." + fldValue.substr(2,2) + "." + fldValue.substr(4);
                        }
 ...
 function getUnlockCommand() {
	...
	for (var i = 0; i < appFormList_GLOBAL.length; i++) {
		...
		if ((unlockStatus != "NO.UNLOCK") && (app && transId)) {
			...
			// Next command should be sepearated by space followed by colon,
			// to avoid inappropriate parsing in OSM.
			unlockCommand += " " + ":" + app + " " + transId + " " + lockDateTime;
		}
	}
...
function retrieveContractData()
{
	...
	var appreqForm = FragmentUtil.getForm(currentForm_GLOBAL);
		...
		if(appreqForm.clientStyleSheet.value != "")
		{
			...
			buildUtilityRequest("","1",currentForm_GLOBAL,"",_OFS__APPLICATION_,"Client Soecific Print","","",false);
...
function setHotField(status, field) {
	...
	if ((hotField != null) && (hotField.getAttribute("hot") != undefined)) {
		if (status == "true") {
			// enable the hotfield
			hotField.hot = "Y";
...
function doFieldChangeEvent(thisEvent)
{
	...
	if ((thisField.getAttribute("type") == "checkbox") && thisField.value == "on")
	{
			// Just return as vaule of the checkbox field "on" indicate just a focus and value is not changed.
				return;
	}
	isAutoEnq = thisField.getAttribute("auto");	
	if( thisField.getAttribute("data-oldValue") == thisField.value ) {
		// Just return as vaule of the field is not changed.
		return;
	}
	else if( thisField.getAttribute("oldValue") == thisField.value ) {
		if (isAutoEnq) {
			thisField.setAttribute("data-oldValue", thisField.value);
		}
		else{
			return;
		}
	...
	if (thisField.getAttribute("id") == resetHotField_Global) {
		// Return to Prevent any further validations as validations are already invoked.
    ...
    var dateFrmt = thisField.getAttribute("dateFormat");
    var fldValue = thisField.value.replace(/^\s+|\s+$/g,"");

    // Check for date format 6 and if so add dot separator while submit the transaction
    if (dateFrmt == "6" && fldValue != "" && fldValue.indexOf(".") == -1 && fldValue.length >= 8 && !isNaN(fldValue.substr(0,8)))
    {
          thisField.value = fldValue.substr(0,2) + "." + fldValue.substr(2,2) + "." + fldValue.substr(4);
    }
	...
	{
		// Set oldValue to prevent any further validations.	
		thisField.setAttribute("data-oldValue", thisField.value);	
	...
	if ( isAutoEnq == "Y" )
	{
		...
		top.autoLaunch = "N";		
		// Set oldValue here so that we can determine if field value is changed.
		thisField.setAttribute("data-oldValue", thisField.value);
	}
...
function preview()
{
	...
	appreqForm.ofsMessage.value = _PREVIEW_;
...
function resizeDiv(event)
{
	...
	var multiRequest = getFormField("generalForm", "multiPane");
	//for enquiry get the exact value of top.noFrames from the form to handle proper resizing.
	var enquiryDataScroller = window.document.getElementById("enquiryDataScroller");
    if ((enquiryDataScroller != null) && (top.noFrames))  //if top.noFrames is false don't care.
	{
		var topNoFrames = window.document.getElementById("topNoFrames"); //we will have this object only if $noFrames is false.
		if(topNoFrames != null || topNoFrames != undefined)
		{
		  top.noFrames = false;
		}
	}
...
function enterKeyListener(e) {   
	...
	if (key == 13) {	//Is Enter key.
		...
		{
			// Get the source element from the event.
			thisField = e.target || e.srcElement;
		}
		...
		if (thisField.getAttribute("data-oldValue") != thisField.value) {
			var isEnterKeyCommit = document.getElementById("defaultButton");
			if (isEnterKeyCommit == null) {
				// proceed for further validations.
				doFieldChangeEvent(e);
			}
			// Prevent any further validations.
			thisField.setAttribute("data-oldValue", thisField.value);
		}
	}	
	return (key != 13);
