...
function signOff()
{
	...
	if ( user != "" )
	{
		buildUtilityRequest("",user,"generalForm","_parent",_DESTROY__SESSION_,"","","","");
		noSignOff = 1;
	}
}
...
function signOffAndCloseWindow()
{
	...
	if ( user != "" )
	{
		...
		var reqType = _DESTROY__SESSION_;
...
function doloadCompany(NewCompanyID)
{
	...
	buildUtilityRequest(_OS__LOAD__COMPANY_,NewCompanyID,"generalForm","NEW","","","","","");
...
function doSwitchRole(NewRole)
{
	...
	buildUtilityRequest(_OS__LOAD__ROLE_,NewRole,"generalForm","NEW","","","","","");
	// Flag for no sign off
	noSignOff = 1;
...
function getCompositeScreenTarget( request)
{
	...
	{	
		...
		if( currentItem )
		{
			...
			if( ( command == _NOENQ_) && ( requestType != _ENQ_ ) )
			{
	...
	else
	{
		switch ( requestType)
	    {
	        case _ENQ_:
	        	...
	        	if(request.indexOf(_DE__PREVIEW_) == 0)
	        	{
