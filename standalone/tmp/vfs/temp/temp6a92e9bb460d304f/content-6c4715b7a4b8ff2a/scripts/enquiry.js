...
var isOpenDocDownloadContentInInternetBrowser = false;
var windowleft = "";
var windowtop  = "";
...
// Required to decide if extra pixels are required for IE Edge browser.
var addPixel = false;
...
function doEnquiryRequest(
	...
	if(FragmentUtil.isCompositeScreen() && _action == _BACK_)
	{
		_target = "s";
	}
	...
	if ( ( prefix == _ENQ_ + " " ) || ( queryPrefix == _QUERY_ + " ") ){
	...
	form.requestType.value = _OFS__ENQUIRY_;
    ...
    var appValue = form.application.value;
	var verValue = form.version.value;
	// Clear the application name and version name while launching a normal enquiry.
	if (!_dropField) {
		form.application.value = "";
		form.version.value = "";
	} else if (appValue == "ENQ" && _dropField == "transactionId" && (enqName.indexOf("$NAU") != -1 || enqName.indexOf("$HIS") != -1)) {
		// clear version name so that enquiry output is not effected by	VERSION.CONTROL setup.
		form.version.value = "";
	} else if ((appValue == "" || verValue == "") && (currentForm_GLOBAL != "enqsel")) {
		// Assign value the application name and version name if null when launched using dropField.
	...
	if(_action != _DRILL_)
	{
		...
		if(clockBox)
		{
			...
			if((_action.indexOf("FAVOU") == -1) && _destination == _DISPLAY_ && clockBox.disabled)
			{
			toggleTimer();		//to stop the timer.
			if(_action == _SELECTION_ && (_args.indexOf("#") != -1))
				{
	...
	if(form["value:999:1:1"] != null && clockBox != null && (_action == _RUN_ || _action.indexOf("FAVOU") != -1))
	{
		form.autoRefresh.value = form["value:999:1:1"].value;
	}
	if (!(form.EnqParentWindow && form.EnqParentWindow.value != '' && form.EnqParentWindow.value != null && FragmentUtil.isCompositeScreen() && _action==_BACK_))
	{
	...
	if(destinationType == _SAVE_ || destinationType == _Save_ || destinationType == _save_)
 	{
		// submit the form, using Fragment population method as necessary
 		FragmentUtil.submitForm(form);
 	}
	else if ( destinationType == _PRINT_ || destinationType == _Print_ || destinationType == _print_ )
	{
		form.download.value ="SERVER.PRINT";
		//change the request type to server print
		form.requestType.value = _OFS__ENQUIRY__PRINT_;
		submitRequest();
		//return the old request type
		form.requestType.value = _OFS__ENQUIRY_	;
	}
	else if ( destinationType == _DISPLAY_ || destinationType == _Display_ || destinationType == _display_ )
	{
		...
		if (_target != "")
		{			
			if (isOpenDocDownloadContentInInternetBrowser && DOC_DOWN_SERVICE && _target == "NEW")
			{				
				myarg = _target;											// if content disposition set to inline we want to show it in new popup window
				currentWinName = FragmentUtil.getWindowOrFragmentName();	// Current window name required later in code so save it here
			}
			// Always want a dropdown to go in to a new window
			else if (_action == _DROPDOWN_)
			{
				myarg = _action;
			}
			else if (_action == _DRILL_)
			{
				myarg = _DRILL_ + " " + _target;
			}
			else if (_action==_BACK_)
			{
				// _action has to be appended so that breadcrumb response will be displayed in correct fragment or window
				myarg = _ENQ_ + " " + enqName + " " + _action;
			}
			else if (_action!="")
			{
				myarg = _target;
			...
			if (recordHistory_GLOBAL == true && MenuStyle == "POPDOWN" && Backevent == false && _action != _DRILL_)
			{
				if(window.BrowserHistory && window.document.getElementById("isARC"))
				{
					var dataArray = {};
			...
			if (isOpenDocDownloadContentInInternetBrowser && DOC_DOWN_SERVICE)
			{
				// in server side doc name is fetched using window name. so window name must be 
				// current window where the drill is placed not the new target window name. Only for inline disposition type.
				form.windowName.value = currentWinName;   
			}

		}
		else
		{
		    if(_action != _DROPDOWN_) //show busy
			  {
		        // Don't set the status to busy, if the action type is  add or delete the favourite.

				if(!( (_action.indexOf(_ADD__FAV_) != 0) || (_action.indexOf(_DEL__FAV_) != 0) ))
					{
						hideEnquiry();
						showState("busy");
					}
		...
		// Check for date format 6 and if so add dot separator while submit the query
		for ( var i = 0; i < form.elements.length; i++ )
		{
			var field = form.elements[i];
			var dateFrmt = field.getAttribute("dateFormat");
			if (dateFrmt && dateFrmt == "6" && field.value != "" && !isNaN(replaceAll(replaceAll(field.value, " ", ""), ".", "")))
			{
				var dateSplitArr = field.value.replace(/^\s+|\s+$/g,"").split(" ");
				var DateCount = dateSplitArr.length;
				var finalStr = "";
				if(DateCount>1){
					for (var k = 0; k < DateCount; k++)
					{
						if(dateSplitArr[k] != '')
						{
							if(dateSplitArr[k].indexOf(".") == -1 )
								finalStr += dateSplitArr[k].substr(0,2) + "." + dateSplitArr[k].substr(2,2) + "." + dateSplitArr[k].substr(4,4);
							else
								finalStr += dateSplitArr[k]; 
							if( k != (DateCount - 1)  )
								finalStr += " ";
						}
					}
					...
					field.value = field.value.substr(0,2) + "." + field.value.substr(2,2) + "." + field.value.substr(4);
				}
			}
		}
		...
		if ( _action == _DROPDOWN_ && (popupDropDown_GLOBAL) )
		{
		...
		else if ( (_action.indexOf(_ADD__FAV_) == 0) || (_action.indexOf(_DEL__FAV_) == 0) )
		{
...
function action( theaction )
{
	...
	else if (theaction==_SELECT_)
	{
		...
		doEnquiryRequest(_SELECTION_, enqname, dropfield, previousEnqs, previousEnqTitles, "", "", "", _DISPLAY_);
	}
	else
	{
		pageNavigation = true;
		doEnquiryRequest(theaction, enqname, dropfield, previousEnqs, previousEnqTitles, "", "", "", _DISPLAY_);
	}
}
...
{
	var mycrumbref = mycrumb;
	// Decode URI if it is received from onclick attribute
	if(getFieldValue("showStatusInfo") == 'false'){
		mycrumbref = decodeURI(mycrumb);
	}
	...
	doEnquiryRequest(_BACK_, mycrumbref, dropField, previousEnqs, previousEnqTitles, "", "", "",	_DISPLAY_);
}
...
function drilldown( item, ref )
{
	...
	if(FragmentUtil.isCompositeScreen())
	{
		...
		if (mydrill == "ENQ")
		{
 		...
 		if(mydrill!="" && mydrill=="AUTOFORM.PDF")
 		{
 			target="NEW";
 		}
		...
		var currentTargetArea = FragmentUtil.getWindowOrFragmentName();

		// Drilldown target passed in request to avoid multiple clicks
		var targetsplit = "";
		if ((form.drilltarget.value!="") && (form.drilltarget.value!=null) && !isInternetExplorer())
		{
        ...
        if ( currentTargetArea.indexOf("_") != -1 )
        {
            currentTarget = currentTargetArea.substring(0,currentTargetArea.indexOf("_"));  //TabName
            currentWindowName = currentTargetArea.substring(currentTargetArea.indexOf("_")+1); //TargetName
        }
        else
        {
            currentTarget = currentTargetArea;
        }
		...
		// Append the target name with the current area window name if it is present
		if (( currentTargetArea.indexOf("_") != -1 ) && (target == currentTarget))
		{
		    target = target+ "_" + currentWindowName;
		}
		//  In noFrames mode, enquiry form objects can be retrieved based on fragment name.
        // So append the currentfragmentname with form element to be searched
        if ((top.noFrames) && (target != currentTarget) && (target.Fragment))
        {
        // Get the current fragment name from the current window usually, but use the target context if supplied
         var windowOrFragment = target.Fragment.getCurrentFragmentName();
		 target = windowOrFragment;
		 }
	}
	else
	{
	...
	if (mydrill==_DOWNLOAD_)
	{
		...
		doEnquiryRequest(_DRILL_, currentEnq, dropField, previousEnqs, previousEnqTitles, args, target, "", _SAVE_);
		DOC_DOWN_SERVICE = false;      // reset the value to false.
    }
    else if (mydrill==_DOC__SERVICE_)
    {
    	// Content needs to be displayed in new popup screen if user wants to display document in Browser itself.    	
		// Otherwise Set no target. Always download dialogue box will be displayed for document download
    	// It avoids blank window display after download also.    	  	
    	if ( getFieldValue("openDocDownloadContentInInternetBrowser") == "true" )
    	{
    		target = "NEW";
    		isOpenDocDownloadContentInInternetBrowser = true;
    	}
    	else{ 
    		target = "";
    	}
		...
		doEnquiryRequest(_DRILL_, currentEnq, dropField, previousEnqs, previousEnqTitles, args, target, "", _DISPLAY_);
        ...
        form.removeChild(drillaction);
   		...
   		isOpenDocDownloadContentInInternetBrowser = false; // reset the value
    }
    else if (mydrill== _COS_)
    {
		doEnquiryRequest(_DRILL_, currentEnq, dropField, previousEnqs, previousEnqTitles, args, target, "", _DISPLAY_);
    }
...
function refresh()
{
	...
	doEnquiryRequest(_SELECTION_, enqname, dropfield, previousEnqs, previousEnqTitles, "LAST", "", "", _DISPLAY_);
}
...
function openNewWindow()
{
	...
	doEnquiryRequest(_SELECTION_, enqname, dropfield, previousEnqs, previousEnqTitles, "LAST#"+pageNumber, "NEW", "", _DISPLAY_);
...
function initEnquiry(enqname, windowTop, windowLeft, windowWidth, windowHeight)
{
	...
	if(FragmentUtil.isCompositeScreen())
	{
    	...
    	if(window.name=="undefined" && parenttarget!=null && parenttarget!='')
		{
			window.name = parenttarget;
		}
    	if (!top.noFrames) {
    		for ( var i = 0; i < window.parent.frames.length; i++ )                          
			{                                                                                
    			var frame = window.parent.frames[i];
    			if(frame.frameElement.name == window.name){
    				if(frame.frameElement.scrolling == "no" && document.getElementById("enquiryDataScroller")){
    					document.getElementById("enquiryDataScroller").style.overflow = "hidden";
    				}
				}
		    }
		}
	...
	//Block back button functionality for composite screens for now and Allow only for ARC-IB
	if (!FragmentUtil.isCompositeScreen() && window.document.getElementById("isARC"))
	 {
	...
	{
		if(reportAction.value == _PRINT__DEAL__SLIP_)
		{
			action(_PRINT_);
		}
	}
	...
	if (isEdge()) addPixel = true;	

	// Resize the enquiryDataScroller elements
	resizeEnquiry();	

	// Resize the report elements
	resizeReport();
	...
	else
	{
		...
		else
		{
			...
			{
				try {
					// The Internet Explorer specific setActive method is similar to the focus method, it activates an element, 
					// but does not try to bring the application window into the foreground.
				    if ( isInternetExplorer() || isEdge() ) {  
				    	enqResheader.setActive();
					} else {
						 enqResheader.focus();				
					}
				} catch( e){/** Do nothing if we cant focus**/};
			}
		}
	}
...
function doEnquiry(downloadFormat)
{
	...
	doEnquiryRequest(_RUN_, name, dropfield, previousEnqs, previousEnqTitles, "", target, "enqsel", downloadType);
}
...
function doDownload(downloadType)
{
	...
	enqselForm.download.value = _ENQUIRY__DOWNLOAD_;
...
function downloadStatement(type, enqName) {
	...
	var select = type;
    doEnquiryRequest(_RUN_, enqName, "", "", "", "", "","enqsel", type);
}
 ...
 */
 function unloadEnquiryWindow() {
	...
	if ( closingEnqWindow && window.enqUnloadArgs && ( ! FragmentUtil.isCompositeScreen() ) ) {
		...
		popupHiddenSubmitWindow( _NO__REQUEST_, args, _ENQ_ + " " + transId, "Y", "Unload" );
	}
}
 ...
 */
	add: function(promptText, errText, helpText) {
		...
		// RE to check for empty or special character 
		var pattern = EnqFavourites.NAME_REGEX;

		// If favourite name does not contain special character then allow it		
		if ((favname.length <= EnqFavourites.NAME_MAX_LEN) && (!pattern.test(favname)) && trimmedFavName.length > 0) {
			...
			var _favAction_ = _ADD__FAV_ + ":" + favname;
            var dropField = enqselForm.dropfield.value; // get the dropfield value
			doEnquiryRequest(_favAction_, curEnqName,dropField, "", "", "", "", "enqsel", _DISPLAY_);
		}
		else {
			alert (errText + '\n\n' + helpText);
		}
 ...
 */
	del: function(favname) {
		...
		if ( defaultButton != null) 								//defaultButton state changed here to avoid form submit
		{
			...
			var enqselForm = FragmentUtil.getForm("enqsel");
			var curEnqName = enqselForm.enqname.value;
			var favAction = _DEL__FAV_ + ":" + favname;
			var dropField = enqselForm.dropfield.value;
			doEnquiryRequest(favAction, curEnqName,dropField, "", "", "", "", "", _DISPLAY_);
		}
	},
 ...
 */
	...
	run: function(favname) {
		...
		var curEnqName = enqselForm.enqname.value;
		var dropField = enqselForm.dropfield.value;
		doEnquiryRequest(_SELECTION_, curEnqName,dropField, "", "", "@" + favname, "", "", _DISPLAY_);
	}
};
...
function resizeEnquiry() {
	...
	var dataDiv = FragmentUtil.getElement("enquiryDataScroller");
	var isTree = dataDiv.getAttribute("isTree");
	var isImImage = dataDiv.getAttribute("isImImage");
	if (isTree || isImImage) {
		return;
	}
...
/**
 * Resizes the report DIV taking into account the body size and content size.
 */
function resizeReport() {
	var reportDiv = document.getElementById("report_response");
	if (reportDiv) {
		// Set the width and height of the report content less than the width and height of the body content, so that scroll bars will be available automatically if the report content gets overflowed.
		reportDiv.style.width = document.getElementsByTagName('BODY')[0].offsetWidth - 20;
		reportDiv.style.height = document.getElementsByTagName('BODY')[0].offsetHeight - 60;
...
function matchSizes(headingRow, dataRow) {
	...
	var interPixelSpacing = 0;
	//Edge uses inter-pixel spacing which results in wrapping of data.So,add extra pixel to compensate this. 	
	if (addPixel) {
		interPixelSpacing = 1;
		addPixel = false;	//Add only once on loading the enquiry.
	}
	...
	for (var i=0; i<columnTotal; i++) {
		...
		if (headingWidth > dataWidth) {
			dataCell.style.width = headingWidth - paddingOffset + interPixelSpacing;
			headingCell.style.width = dataCell.style.width;
		} else {
			headingCell.style.width = dataWidth - paddingOffset + interPixelSpacing;
