...
function FieldGroup( id) {
	...
	//variable set(flagged) when there is flow of same instance id across multiple tabs.
	this.multiTabInstance = "";
...
FieldGroup.prototype.collapse = function( instanceId)
{
    ...
    if (!this.currentTab) {	// check if we already know the current tab.
		if (appTabList_GLOBAL)
		{                                                                                                                             
			for( var i = 0; i < appTabList_GLOBAL.length; i++)
			{
		...
		{
		//check if there is flow of single sub value instance across multiple tabs for eg,PP.CONTRACT,AMEND
			if (this.groupType == "S") {
				for( var i = 0 , j = 0 ; i < tabList_GLOBAL.length; i++)
				{
					// Set the tab to check.
					this.currentTab = tabList_GLOBAL[i];
	        		this.currentInstance = this.getInstanceAt( instanceId );
	       			// Validation
	        		if(this.currentInstance[0] != null)  {
	       	    	...
	       	    	if( j > 1 ) {    
	       	    		this.multiTabInstance = 1; //set the flag.
	        			break;
	        		}
	    		}
			}
...
/**
 *  while Deleting Multi Langual field changed values
 *  are added to form
 */
    var isLanguageField = this.isLanguageExpansion( this.currentInstance[0], instanceId); //check if current instance is language field.
 	if(isLanguageField)
	{
	...
	if( instanceId == this.firstInstanceId )
	{
		...
		if( thisInstance[0] != null)
		{
			...
			{ 
				...
				if ((rowElement != null && rowElement != undefined) && (oldRowElement != null && oldRowElement != undefined)) {					
	...
	for( var i = 0; i < aInstance.length; i++)
	{
		...
		for( var j = 0; j < tmpArr.length; j++)
		{
			...
			if( type == "text" )
			{
                ...
                if(subdetails!=null) {
					   subvalueid = subdetails.getAttribute("svdetails");
				} 
				else {				
				 	//for multiTabInstance svdetails is available only for the tab where sub value delete button was clicked. So,
				 	//use "svlist" instead.
				 	if (this.multiTabInstance == 1) {  
						subvalueid =aInstance[i].getAttribute("svlist"); //use attribute "svlist" if "svdetails" is not available.
				 	}
				}
...
FieldGroup.prototype.updateInputElementValues = function( element)
{
	...
	{	
		...
		//check if its multiTabInstance as we have to take tabs into consideration while updating values.
		//Bypass the "select-one" type of elements as they are unique even in case of multiTabInstance.
		if ((this.multiTabInstance == 1) && (element.type != "select-one")) { 	
			var form = FragmentUtil.getForm(currentForm_GLOBAL);
		    var field = form.elements[nextElementName];  //get all the fields with name eq nextElementName.
		    //now get the field that belongs to current tab and update the elementlist.
			if(field != null) {
				for(i=0; i<field.length; i++) {
					var tabName = field[i].getAttribute("tabname");
					if (tabName == this.currentTab) {
						elementList[ elementList.length] = field[i];
					}
				}
			}
		    else {
			   nextElement = null;  //there is no field to update ,so get out of while loop.
			}
		}
		else {
...
FieldGroup.prototype.initMetaGroupSet = function( instanceId)
{
	...
	for( var i = 0; i < this.metaGroupSet.length; i++)
	{
		...
		for( var j = 0; j < inputElements.length; j++)
		{
			...
			inputElements[j].setAttribute("data-oldValue","");
		...
		{
			var mandatory_field = false;
			var select_previous = selectElements[j].parentNode.previousSibling.getElementsByTagName("img");
			// Loop through previous elements and find whether is mandatory field.
			if(select_previous)
			{
				for(var k = 0; k < select_previous.length; k++)
				{
					if( select_previous[k].title == "Mandatory Field" )
					{
			...
			if(mandatory_field)
			{
				selectElements[j].selected = "true";
				selectElements[j].selectedIndex=0; //set first value as default for mandatory field.
				selectElements[j].setAttribute("oldValue","");
				selectElements[j].setAttribute("data-oldValue","");
			}
			else
			{
		...
		for( var j = 0; j < textAreaElements.length; j++)
		{
			...
			textAreaElements[j].setAttribute("data-oldValue","");
		}
		...
		for( var j = 0; j < spanElements.length; j++)
		{
			...
			else if(spanId != null && spanId != "" && spanId.indexOf("disabled_") > -1 )
            {
				...
				spanElements[j].setAttribute("data-oldValue",""); 
