...
Fragment.prototype.populateWithURL = function(URL, params, noBusy, noRetries) {
	...
	if (req) {
		...
		this.req = req;
		//get Field id from the request
        var reqFieldId = this.id;
        //check field id contains tabpane which indicates tab expansion                
        var tabPos = reqFieldId.indexOf("tabPane");
        // check for fieldName:ID.COMP.6 in field id which refers relative calendar used in AA   
        var relReq = reqFieldId.indexOf("fieldName:ID.COMP.6");
		...
		if (params == null) {
			...
			try {
				var reqFlag = false;
				// asynchronous mode for drop down and multiple request sent from single frame.
				if ((relReq > -1) || (tabPos > -1) || currentDropDown_GLOBAL) {
					reqFlag = true;
				}

				if (reqFlag)
				{
					this.req.open('POST', URL, reqFlag);
				...
				{
					// 1 millisecond delay has been introduced to help Internet Browser to display the busy window while synchronous request is posted.					
					var tempThis = this;
					setTimeout (function() {
						tempThis.req.open('POST', URL, reqFlag);
						tempThis.req.setRequestHeader("Content-type", "application/x-www-form-urlencoded;charset=UTF-8");// set the charset for encoding to work in IE	
						tempThis.req.setRequestHeader("Content-length", params.length);
						tempThis.req.setRequestHeader("Connection", "close");
						tempThis.req.send(params);
					}, 1);
				}
			}
			catch (exception) {
...
Fragment.prototype._populateWithRequest = function(req) {
	if (req.status == 200 || req.status == 0) { // status 0 required to support file protocol	
		...
		// Check whether the response is login page content with j_security_check implementation
		// if so then it the response for a request with invalid session.
		if ((htmlText.indexOf("j_security_check") > -1) && (htmlText.indexOf("doFormBasedAuthentication") > -1) && (htmlText.indexOf("FtressUserIdForm") > -1)) {
...
Fragment.sendRequest = function(URL, params, callbackFunc, noRetries) {
	...
	if (req) {
		...
		try {
			if (params) {
				req.open('POST', URL, false); //enable synchronous request to avoid session timeout in IE
