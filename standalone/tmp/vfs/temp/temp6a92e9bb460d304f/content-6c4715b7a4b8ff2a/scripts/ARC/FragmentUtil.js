...
FragmentUtil = {
    ...
    submitForm  : function(form, windowRef) {
        ...
        if (!top.noFrames || (typeof DOC_DOWN_SERVICE != "undefined" && DOC_DOWN_SERVICE)) {
        	// Target is required when user wants to display the doc down service content in Browser itself.   
        	if(typeof DOC_DOWN_SERVICE != "undefined" && DOC_DOWN_SERVICE && !isOpenDocDownloadContentInInternetBrowser){
        		actualForm.target = "";
        	}        	
            actualForm.submit();
        }
...
//			     	}
    ...
    getFrameWindow  : function(target) {
        ...
        if (top.noFrames) {
            // Do this properly, as for frames ..
            return window;
        }
        else {
    ...
    checkKeepalive : function() { 
       ...
       if (! FragmentUtil.userActivity) {
            ...
            if  ( (!FragmentUtil.userActivity) && ( top.isWorkspace !=true) ) {
                // User has cancelled - redirect to the login page
                var baseURL = window.location.protocol + "//" + location.host + "/" + getWebServerContextRoot()+ "/jsps/timeout.jsp";
...
/*
 * Method to submit a utility request without any delay.
 * @return {void}
 * 
 */
function submitUtilityRequest()
{
	var currentFragmentName = FragmentUtil.getWindowOrFragmentName();
	var utilityRequestForm = FragmentUtil.getForm("utilityRequestForm");
	if(utilityRequestForm !=null)
	{
		utilityRequestForm.windowName.value = currentFragmentName;
		FragmentUtil.submitForm("utilityRequestForm");	
	}
