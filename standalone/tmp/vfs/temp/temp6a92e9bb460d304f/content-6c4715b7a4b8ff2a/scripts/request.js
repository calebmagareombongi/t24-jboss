		...
		//Fast path enquiry to have parent window name 
  		if (routineName == "OS.FAST.PATH")
  		{
			...
			if (form.EnqParentWindow != null || form.EnqParentWindow != '') {
				// we fetch the fastpath details with enquiry parent window name in t24 so ensure always EnqParentWindow is set correctly. 
...
function createResultWindow2(cmdname, width, height, page)
{
	...
	// Popup dropdown enquiry should open in new window always
	if (( enquiryType == _CONTEXT__ENQUIRY_ ) && ( compScreen != "" ) && ( cmdname != _DROPDOWN_ ) && ( prefix != _DRILL_ ) && ( breadcrumbAction !=_BACK_ ) && ( requestRoutineName != "OS.GET.PASSBOOK.PRINT.DATA" ))
	{
	...
	if ( compTarget && compTarget != "NEW" )
	{
		...
		if (windowName == currentwindow) {
			...
			var content = '<html><head><link rel="icon" href="../plaf/icon/favicon.ico"/><title>processing...</title><meta http-equiv="Content-Type" content="text/html; charset=utf-8"></head><body style="background-color: #e0ebea"; oncontextmenu="return false;"><div style="width: 100%; height: 100%; background: url(../plaf/images/default/gears.gif) no-repeat center center;"></div></body></html>';
			for (var i = 0; i < parent.frames.length; i++) {
				try {
					if (parent.frames[i].name == windowName) {
						if (parent.frames[i].document.getElementById("processingPage")) {
							parent.frames[i].document.getElementById("processingPage").style.display = "block";
						} else {
