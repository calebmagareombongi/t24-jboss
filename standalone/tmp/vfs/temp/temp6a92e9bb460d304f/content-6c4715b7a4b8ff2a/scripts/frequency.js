...
function dropFrequencyDisplay(dropFieldId)
{
    ...
    genForm.routineName.value = _OS__GET__FREQUENCY_;
...
function initFrequency(dropFieldId, fragmentName) {
	...
	if (fquFocus != undefined) {
		scrollIntoViewIfRequired(fquFocus);
	}
}
...
function fqu_saveDate(dropFieldId)
{
	...
	var dateFrmt = document.getElementById("fqu:nextDate").getAttribute("dateFormat");

	// Check for date format 6 and if so add dot separator while submit the transaction
	if (dateFrmt && dateFrmt == "6" && nextDate != "" && nextDate.indexOf(".") == -1)
	{
		nextDate = nextDate.substr(0,2) + "." + nextDate.substr(2,2) + "." + nextDate.substr(4);
	}
...
function calendarFrequencyHide(theEvent)
{
	...
	if ( currentDropDown_GLOBAL || freqDropDown_GLOBAL )
	{
		...
		// Dropdown to close when the input field is clicked after opening the dropdown
		var fieldtagName = thisField.tagName;
		...
		if ( currentDropDown_GLOBAL && !freqDropDown_GLOBAL )
		{
			...
			{
				// When only the calendar dropdown is opened, close the dropdown when user clicks on the input text area
		...
		if ( currentDropDown_GLOBAL && freqDropDown_GLOBAL && currentDropDown_GLOBAL != freqDropDown_GLOBAL )
		{
			...
			{	
				...
				if ( calendarFieldId != null ) 
				{
					// When there are nested controls, close the inner window when input text area in the inner control is clicked.
					if(fieldtagName != "INPUT")
					{
