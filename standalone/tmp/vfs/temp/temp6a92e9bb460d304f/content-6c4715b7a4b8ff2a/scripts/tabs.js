 ...
 * @return {void}
 */ 
function gettabbedscreen(routine,cmdname, tabId,contentType, formName, frameName,workareaTransId)
{
	...
	if ( top.noFrames && currentFragment_GLOBAL )
	{
    	...
    	genForm.routineName.value = _OS__PROCESS__TAB_;
    	if(contentType == 'ENQ')
    	  {
    	  ...
    	  {
		  	genForm.routineArgs.value = cmdname+"_"+tabNumber+"_"+workareaTransId;
		  }
	...
	if(windowName==null || windowName=="" )
	{
		...
		var user = getUser();
		//We need user role value as well for maintaining consistent SMS on switching tabs.
		var operatorRole = document.getElementById("userRole").value; 
		var jspRequest="../jsps/genrequest.jsp?&routineName="+routine+"&skin="+getSkin()+"&compId="+companyId+"&compScreen="+getCompositeScreen()+"&contextRoot=/"+contextRoot+"&windowName="+windowName+"&user="+user+"&usrRole="+operatorRole;
		// Add form token parameter to request.
		if(arguments.length > 7) {
			var tokenParam = arguments[7];
			if(tokenParam != "" && tokenParam != _ISSUE__TOKEN_){
				jspRequest += tokenParam;
			}
		}
		if (contentType == "ENQ") 
		{
...
//	params += "&WS_FragmentName=" + fragmentName; // TODO: Do we need this?
    ...
    // to avoid security vulnerability & is converted to ~Amp# in params
