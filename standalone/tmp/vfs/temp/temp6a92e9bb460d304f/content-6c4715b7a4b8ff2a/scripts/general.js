...
function getFormField( formName, fieldName )
{
	...
	if (field != null) {
		try
		{
			if ( ( field.length > 1 ) && ( field.type != "select-one" ) )
			{
				if (currentTabName_GLOBAL) {
					//When a field instance is found twice in the same tab then take the one that matches current tab name
					for ( var i = 0; i < field.length ; i++ )
					{
						if ( field[i].getAttribute("tabname") == currentTabName_GLOBAL )
						{
...
{
	if(!FragmentUtil.checkWindowChanges(_DEACTIVE__PROFILE_))
	{
		return;
	}
	// Send utility request to server to build the change password form
	buildUtilityRequest(_OS__PASSWORD_,_BUILD__DEACTIVATION__PROFILE_,"generalForm","","","Deactivate Profile","","","");
}
...
function SendDeactivateInfo()
{
	...
	if ((action == _PROCESS__DEACTIVATION__PROFILE_ ))
	{
...
function doutil(myutil)
{
	...
	if(arguments.length > 1) {
		tokenParam = arguments[1];
	}

	if(myutil==_CHANGE__PASS_)
	{
		changePass();
	}
	else if(myutil==_DEACTIVE__PROFILE_)
 	{
 		doDeactivation();
 	}
	else if(myutil==_DISABLE__PROFILE_)
	{
		disablePass();
	}
	else if(myutil==_QUICK__GUIDE_)
	{
		quickGuide();
	}
	else if(myutil==_T24__HELP_)
	{
		help( "", "", "", tokenParam );
	}
	else if(myutil==_ABOUT__BROWSER_)
	{
		about(tokenParam);
	}
...
function setPassword()
{
	...
	if ( ( action == _PROCESS__REPEAT_ ) || ( action == _PROCESS__EXPIRED_ ) )
	{
	...
	else if ( action == _PROCESS__CHANGE_ )
	{
...
function changePass()
{
	...
	if(!FragmentUtil.checkWindowChanges(_CHANGE__PASS_))
	{
		return;
	}
	// Send utility request to server to build the change password form
	buildUtilityRequest(_OS__PASSWORD_,_BUILD__CHANGE_,"generalForm","","","Change Password","","","");
}
	...
	if ( fieldTabName != null )
	{
		if (fieldTabName != "")
		{
			...
			if ( ( fieldTable != null ) && ( fieldTable.style.visibility != "hidden" ) && ( fieldTableDisabled == "false" ))
			{
				...
				if(trDetails != '')
				{
					// If the table data is not empty in IE then take the 1st childNode, else take the
					// second node for processing
					if(isInternetExplorer() && trDetails.childNodes[0].className != "empty")
					{
						actionContainerDets = trDetails.childNodes[1];
					}
					else
					{
						actionContainerDets = trDetails.childNodes[2];
					}
					if(actionContainerDets.childNodes.length != 0)
					{
						if(actionContainerDets.childNodes[0].tabIndex == 0)
						{
							// get the action_container for the field to be focused
							var actionContainerDetsTemp = FragmentUtil.getElementsByClassName("action_container",'td',focusFld.parentNode.parentNode);
							if(actionContainerDetsTemp.length > 0)
							{
								actionContainerDets = actionContainerDetsTemp;
							}
...
function dotabbedscreen(itemname){
	...
	enqselForm.routineName.value=_OS__GET__TABBED__SCREEN__XML_;
	enqselForm.routineArgs.value=cmdname+"|"+itemname;
	enqselForm.requestType.value = _UTILITY__ROUTINE_;
...
function setFormFieldValue( _formId, _fieldName, _fieldValue )
{
	...
	for ( var i = 0; i < form.elements.length; i++ )
	{
        ...
        var frqFld = field.getAttribute("frqFld");
        var fldValue = _fieldValue.replace(/^\s+|\s+$/g,"");

        if ( field.name == _fieldName )
        {
              // Check for date format 6 in frequency field during web validation.
              if (frqFld == "Y" && dateFrmt == "6" && fldValue != "" && fldValue.indexOf(".") == -1 && fldValue.length >= 8 && !isNaN(fldValue.substr(0,8)))
              {
                    field.value = fldValue.substr(6,2) + "." + fldValue.substr(4,2) + "." + fldValue.substr(0,4) + " " + fldValue.substr(8).replace(/^\s+|\s+$/g,"");
              }
              else
              {
...
function sendMaintenanceInfo()
{
	...
	password = replaceAll(password, ":", "&#58;");
	if ((action == _PROCESS__CORRECTION__REPEAT_ ))
	{
...
function setFocus()
{
	...
	else if(action==_PROCESS__DEACTIVATION__PROFILE_)
 	{
...
/**
 * Return true if the browser is Edge, else return false
 * @return 'TRUE' or 'FALSE'
 */
function isEdge(){
	var isEdge = false;
...
/**
 * Return true if the browser is Chrome, else return false
 * @return 'TRUE' or 'FALSE'
 */
function isChrome(){
...
function formatTextArea( fieldName, size)
{
	...
	var tokens = "";
	if ( (isInternetExplorer() && (navigator.appVersion.indexOf("MSIE 10") == -1)) || isOpera() ) {
	...
	var newValue = "";
	if (isFirefox() || (navigator.appVersion.indexOf("MSIE 10") != -1)) {
...
function clearField( fieldName)
{
	...
	clearOldValues(currentForm_GLOBAL, fieldName);
}
...
function getCurrentTabFieldValue( formName, fieldName )
{
	...
	// refresh current tab list as user might moved anywhere in AA screen.
...
function updateFormFieldValueOnCurrentFragment(field_Name, field_Value)
{
	...
	var currentDivObjects = null;

	if(currentFragmentObject){
		currentDivObjects = currentFragmentObject.getElementsByTagName("form");
	}
...
// Scroll into visible area only if required.
function scrollIntoViewIfRequired(ddFocus) {
	var rect = ddFocus.getBoundingClientRect();
	if (rect.bottom > window.innerHeight) {
		ddFocus.scrollIntoView(false);
	}
} 
//Clear data-oldvalues and oldvalues.
function clearOldValues( _formId, _fieldName) {
	if (_formId == "")
	{
		_formId  = parent.currentForm_GLOBAL;
	}
	...
	for ( var i = 0; i < form.elements.length; i++ )
	{
        ...
        if ( field.name == _fieldName ){
			...
			field.setAttribute("oldValue","");
			field.setAttribute("data-oldValue","");
		}	
	}     
}