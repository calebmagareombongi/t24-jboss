...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="responseDetails">
		<form name="changePassword" id="changePassword" method="POST" action="BrowserServlet" AUTOCOMPLETE="OFF">
			<xsl:variable name="lngDir"><xsl:value-of select="/responseDetails/userDetails/lngDir"/></xsl:variable>
            <xsl:if test="$lngDir!=''">
                <xsl:attribute name="dir"><xsl:value-of select="$lngDir"/></xsl:attribute>
            </xsl:if>
			...
			<div class="display_box">
			    <xsl:attribute name="float"><xsl:choose><xsl:when test="$lngDir='RTL'">right</xsl:when><xsl:otherwise>left</xsl:otherwise></xsl:choose></xsl:attribute>
				<!-- Main form table -->
				<table cellSpacing="0" cellPadding="0" border="0" align="center" style="background-color:#ffffff;">
					<tbody>
						...
						<xsl:for-each select="r">
							<tr>
							<xsl:attribute name="text-align"><xsl:choose><xsl:when test="$lngDir='RTL'">right</xsl:when><xsl:otherwise>left</xsl:otherwise></xsl:choose></xsl:attribute>                             
						        <td>
