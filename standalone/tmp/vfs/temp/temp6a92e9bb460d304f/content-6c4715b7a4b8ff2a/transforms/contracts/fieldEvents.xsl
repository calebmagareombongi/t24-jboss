...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
    ...
    <!--  ********************************************************************************************* -->
		...
		<xsl:variable name="isCheckBox">
			<xsl:if test="./ty='checkbox'">
		 		<xsl:value-of select="'Y'"/>
			</xsl:if>
		</xsl:variable>
		...
		<xsl:choose>	
			<xsl:when test="($isHot = 'Y') or ($isHotVal = 'Y') or ($isWebVal = 'Y') or ($isAutoEnq = 'Y') or ($isCheckFileEnri = 'Y') or ($isCaseConv!='') or ($isFrequency = 'Y')">
                ...
                <xsl:attribute name="onKeyDown">enterKeyListener(event);</xsl:attribute> 				
				<xsl:choose>
				<xsl:when test="($isCheckBox = 'Y')">
					<xsl:attribute name="onBlur">javascript:disableFocus(this); FragmentUtil.formChangeHandler(); FragmentUtil.fieldChangeHandler(event); doFieldChangeEvent(event);</xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
					<xsl:attribute name="onBlur">FragmentUtil.formChangeHandler(); FragmentUtil.fieldChangeHandler(event); doFieldChangeEvent(event);</xsl:attribute>
				</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:otherwise>
				...
				<xsl:choose>
				<xsl:when test="($isCheckBox = 'Y')">
					<xsl:attribute name="onBlur">javascript:disableFocus(this); clearEnrichment(event); FragmentUtil.formChangeHandler(); FragmentUtil.fieldChangeHandler(event);</xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
