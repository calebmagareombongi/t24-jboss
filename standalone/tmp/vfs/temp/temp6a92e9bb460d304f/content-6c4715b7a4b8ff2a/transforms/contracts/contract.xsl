...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="histNo" select="string(/responseDetails/window/panes/pane/contract/histNo)"/>
...
<!-- Here we put the div to show the errors, etc. -->
		<xsl:choose>
			...
			<!-- height and width has to be mentioned in min-height,min-width with auto style for fire fox to auto adjust the div border in contract screen-->
 			<div onscroll="javascript:testmouse(event);" id="contract_screen_div">
                <xsl:choose>
                     <xsl:when test="$stripFrameToolbars = 'true'">
                         <xsl:attribute name="style">width:99%; height: auto</xsl:attribute>
                     </xsl:when>
                     <xsl:otherwise>
                          <xsl:attribute name="style">width:99%;min-height:50px;height: auto</xsl:attribute>
                     </xsl:otherwise>
                </xsl:choose>
	...
	<xsl:template match="moreactions" name="moreactions">
		...
		<td>
			<a onfocus ="focusonKey('inline',event);" onblur = "hideTooltip(event);" title="Go">
				<xsl:choose>
					<xsl:when test="$showStatusInfo='false'">
						<xsl:attribute name="onclick">javascript:doMoreActions()</xsl:attribute>
						<xsl:attribute name="href">javascript:void(0)</xsl:attribute>
					</xsl:when>
					<xsl:otherwise>
						<xsl:attribute name="href">javascript:doMoreActions()</xsl:attribute>
					</xsl:otherwise>
				</xsl:choose>
				<xsl:attribute name="class">
					<xsl:call-template name="apply_Style">
				...
				<td nowrap="nowrap">
					...
					<xsl:choose>
						...
						<xsl:otherwise>
								...
								<!-- Add histNo in form for Browser to send histNo in copy request-->
								<xsl:if test="$histNo!=''">
									<input type="hidden" name="histNo" id="histNo" value="{$histNo}"/>
								</xsl:if>
					...
					<xsl:choose>
						<xsl:when test="key=''">
							<td width="1">
								<a onfocus ="focusonKey('inline',event);" onblur = "hideTooltip(event);">
									<xsl:choose>
										...
										<xsl:otherwise>
											<xsl:attribute name="href">javascript:dropdown('<xsl:value-of select="app" /><xsl:value-of select="version" />', 'transactionId')</xsl:attribute>
										</xsl:otherwise>
									</xsl:choose>
								...
								<a onfocus ="focusonKey('inline',event);" onblur = "hideTooltip(event);">
								   ...
								   <xsl:attribute name="title">Selection Criteria</xsl:attribute>
									<xsl:choose>
										...
										<xsl:otherwise>
											<xsl:attribute name="href">javascript:dropdown('<xsl:value-of select="app" /><xsl:value-of select="version" />', 'transactionId','','','Yes')</xsl:attribute>
										</xsl:otherwise>
									</xsl:choose>
								   <xsl:attribute name="class">
										<xsl:call-template name="apply_Style">
							...
							<xsl:if test="allowNewDeal='yes'">
								<td width="1">
									<a onfocus ="focusonKey('inline',event);" onblur = "hideTooltip(event);">
									<xsl:attribute name="accesskey"><xsl:value-of select="/responseDetails/window/translations/sKey/newDeal"/></xsl:attribute>
										<xsl:choose>
											<xsl:when test="$showStatusInfo='false'">
											...
											<xsl:otherwise>
												<xsl:attribute name="href">javascript:doNewDeal()</xsl:attribute>
											</xsl:otherwise>
										</xsl:choose>
										<xsl:attribute name="tabindex">0</xsl:attribute>
