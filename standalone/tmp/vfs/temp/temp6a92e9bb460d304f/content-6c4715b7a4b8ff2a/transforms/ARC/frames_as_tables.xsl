...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="frameset_with_rows">
		...
		<xsl:for-each select="frame|frames">
			...
			<xsl:attribute name="height">
		  	...
		  	<xsl:attribute name="align"><xsl:choose><xsl:when test="/responseDetails/userDetails/lngDir='RTL'">right</xsl:when><xsl:otherwise>left</xsl:otherwise></xsl:choose></xsl:attribute>
	...
	<xsl:template match="frame">
		...
		<xsl:variable name="args">
			<xsl:call-template name="encode-slash-plus">
            ...
            <xsl:when test="routine!=''">
            	...
            	<xsl:choose>
            		<xsl:when test="routine='OS.GET.COMPOSITE.SCREEN.XML'">
            			<xsl:attribute name="fragmenturl">BrowserServlet?method=post&amp;user=<xsl:copy-of select="$user"/>&amp;windowName=<xsl:value-of select="n"/>&amp;WS_FragmentName=<xsl:value-of select="$id"/>&amp;contextRoot=<xsl:copy-of select="$contextRoot"/>&amp;companyId=<xsl:copy-of select="$companyid"/>&amp;userRole=<xsl:value-of select="$userrole"/>&amp;compScreen=<xsl:copy-of select="$compScreen"/>&amp;command=globusCommand&amp;skin=<xsl:copy-of select="$skin"/>&amp;requestType=UTILITY.ROUTINE&amp;routineName=<xsl:value-of select="routine"/>&amp;routineArgs=<xsl:value-of select="$args"/>&amp;WS_replaceAll=<xsl:value-of select="replaceAll"/>&amp;WS_parentComposite=<xsl:value-of select="$id"/>&amp;formToken=<xsl:value-of select="$csrfToken"/></xsl:attribute>
            		</xsl:when>
    ...
    <xsl:template name="encode-slash-plus">
        <xsl:param name="string"/>
        <xsl:variable name="bslash" select='"\"'/>
