...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <!-- Base characters to detect Arabic by subtracting these from a word. -->
    <xsl:variable name="replaceChars">abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789,;?.!:%$-[]{}()#^~`*/+-=">@_|\,'&lt;&amp;</xsl:variable>
    
    <!-- Extract CSRF Token -->
    <xsl:variable name="csrfToken" select="/responseDetails/userDetails/csrfToken"/>
        ...
        <xsl:attribute name="class">
          <xsl:call-template name="apply_Style">
                        ...
                        <tr height="15">
                                        ...
                                        <a>
                                            ...
                                            <xsl:choose>
                                                <xsl:when test="$showStatusInfo='false'">
                                                    <xsl:attribute name="onclick">javascript:doFPCommit()</xsl:attribute>
                                                    <xsl:attribute name="href">javascript:void(0)</xsl:attribute>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                    <xsl:attribute name="href">javascript:doFPCommit()</xsl:attribute>
                                                </xsl:otherwise>
                                            </xsl:choose>
                            ...
                            <xsl:if test="(/responseDetails/window/panes/pane/dataSection/enqResponse/multiDownload ='YES')">
                                <td>
                                    <a>
                                        <xsl:choose>
                                            <xsl:when test="$showStatusInfo='false'">
                                                <xsl:attribute name="onclick">javascript:doMDCommit()</xsl:attribute>
                                                <xsl:attribute name="href">javascript:void(0)</xsl:attribute>
                                            </xsl:when>
                                            <xsl:otherwise>
                                                <xsl:attribute name="href">javascript:doMDCommit()</xsl:attribute>
                                            </xsl:otherwise>
                                        </xsl:choose>
                            ...
                            <td>
                                <a accesskey="">
                                    <xsl:choose>
                                        <xsl:when test="$showStatusInfo='false'">
                                            <xsl:attribute name="onclick">javascript:toggleitemdisplay('enqinfo');</xsl:attribute>
                                            <xsl:attribute name="href">javascript:void(0)</xsl:attribute>
                                        </xsl:when>
                                        <xsl:otherwise>
                                            <xsl:attribute name="href">javascript:toggleitemdisplay('enqinfo');</xsl:attribute>
                                        </xsl:otherwise>
                                    </xsl:choose>
            ...
            <xsl:choose>
                ...
                <xsl:choose>
                    <!-- REPORT-->
                    <!-- Since page break will not work properly inside positioned elements, reports are handled seperately. -->
                    <xsl:when test="r/c/rpt!=''">
                        <pre id="report_response">
                            <xsl:attribute name="class">
                                <xsl:call-template name="apply_Style">
                            ...
                            <xsl:choose>
                                <xsl:when test="r/c/rpt/sp!=''">
                                    <xsl:for-each select="r/c/rpt/sp">
                                        <p>
                                            <xsl:if test="position()!=last()">
                                                <xsl:attribute name="class">page_break</xsl:attribute>
                                            </xsl:if>
                                            <xsl:choose>
                                                <xsl:when test="/responseDetails/userDetails/lngDir='RTL'">
                                                    <xsl:call-template name="concat-rlm-mark">
                                ...
                                <xsl:otherwise>
                                    <xsl:choose>
                                        <xsl:when test="/responseDetails/userDetails/lngDir='RTL'">
                                            <xsl:call-template name="concat-rlm-mark">
                                                <xsl:with-param name="report" select="r/c/rpt" />
                                            </xsl:call-template>
                                        </xsl:when>
                                        <xsl:otherwise>
                                            <xsl:value-of select="r/c/rpt" />
                                        </xsl:otherwise>
                                    </xsl:choose> 
                                </xsl:otherwise>
                            </xsl:choose>
                            ...
                            <table id="enquiryResponseData{$formFragmentSuffix}">
                                ...
                                <xsl:choose>
                                    ...
                                    <xsl:when test="/responseDetails/window/panes/pane/dataSection/enqResponse/control/graphImage">
                                        <tr>
                                            <td>
                                                <img src="GraphServlet?dataLocator={/responseDetails/window/panes/pane/dataSection/enqResponse/control/graphImage}&amp;formToken={$csrfToken}"/>
                                            </td>
                                        </tr>
                                    </xsl:when>
                                    <xsl:otherwise>
            ...
            <td>
                ...
                <div id="enquiryDataScroller{$formFragmentSuffix}" onscroll="scrollHeading();">
                    <xsl:if test="(r/tree)">
                     ...
                     <xsl:if test="(r[1]/c[1]/ImDocLoc) and (r[1]/c[1]/img)">
                                 <xsl:attribute name="isImImage">YES</xsl:attribute>
                    </xsl:if>
                    <xsl:attribute name="class">
                        <xsl:call-template name="apply_Style">
                        ...
                        <td>
                            <xsl:if test="$qref!=''"> <!-- The drilldown icon shouldn't be displayed on blank rows (TTS0707203) -->
                                <a>
                                    <xsl:choose>
                                        <xsl:when test="$showStatusInfo='false'">
                                            <xsl:attribute name="onclick">javascript:view('<xsl:value-of select="item"/>','<xsl:value-of select="$qref"/>')</xsl:attribute>
                                            <xsl:attribute name="href">javascript:void(0)</xsl:attribute>
                                        </xsl:when>
                                        <xsl:otherwise>
                                            <xsl:attribute name="href">javascript:view('<xsl:value-of select="item"/>','<xsl:value-of select="$qref"/>')</xsl:attribute>
                                        </xsl:otherwise>
                                    </xsl:choose>
                ...
                <!--Set the align attribute if the field has right justification. -->
                    ...
                    <xsl:if test="endtree">
                        <a>
                            <xsl:choose>
                                <xsl:when test="$showStatusInfo='false'">
                                    <xsl:attribute name="onclick">javascript:expandrow(<xsl:value-of select="../st"/>,<xsl:value-of select="../no"/>)</xsl:attribute>
                                    <xsl:attribute name="href">javascript:void(0)</xsl:attribute>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:attribute name="href">javascript:expandrow(<xsl:value-of select="../st"/>,<xsl:value-of select="../no"/>)</xsl:attribute>
                                </xsl:otherwise>
                            </xsl:choose>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test="ed!=''">
                            <xsl:choose>
                                <xsl:when test="ed/noEdit">
                                    <input type="hidden" value="{ed/val}" size="{i}" id="{ed/n}" name="{ed/n}" therow="{../ref}" tabname="tab1"/>
                                ...
                                <xsl:otherwise>
                                    <xsl:choose>
                                        <xsl:when test="(/responseDetails/window/panes/pane/dataSection/enqResponse/fastPath ='YES')">
                                            ...
                                            <xsl:choose>
                                                <xsl:when test= "$testRow != ''">
                                                    <input type="text" value="{ed/val}" size="{i}" id="{ed/n}" name="{ed/n}" therow="{../ref}" mvDetails="{ed/mvDetails}" tabname="tab1" onkeyPress="javascript:stopPoll()" onChange="javascript:setchecked('chk{../ref}')"/>
                                                </xsl:when>
                                                <xsl:otherwise>
                                        ...
                                        <xsl:otherwise>
                                            <input type="text" value="{ed/val}" id="{ed/n}" size="{i}" name="{ed/n}" therow="{../ref}" tabname="tab1" onkeyPress="javascript:stopPoll()" onChange="javascript:setchecked('chk{../ref}')"/>
                                        </xsl:otherwise>
                                    </xsl:choose>
                                </xsl:otherwise>
                            </xsl:choose>
                            ...
                            <xsl:if test="substring-after(link,'[')!=''">
                                <a>
                                    ...
                                    <xsl:choose>
                                        ...
                                        <xsl:otherwise>
                                            <xsl:attribute name="href">javascript:docommand('<xsl:value-of select="substring-before(link,'[')" />')</xsl:attribute>
                                        </xsl:otherwise>
                                    </xsl:choose>
                                    <img>
                            ...
                            <xsl:if test="substring-after(link,'[')=''">
                                <a>
                                    <xsl:choose>
                                        ...
                                        <xsl:otherwise>
                                            <xsl:attribute name="href">javascript:docommand('<xsl:value-of select="link" />')</xsl:attribute>
                                        </xsl:otherwise>
                                    </xsl:choose>
                        ...
                        <xsl:when test="sound !=''">
                         <xsl:variable name="jinglePath">
                          <xsl:choose>
                            ...
                            <!-- Populate URL to cloud location if useCloudImage is set and server option enabled in image type -->
                            <a>
                                <xsl:choose>
                                    <xsl:when test="$showStatusInfo='false'">
                                        <xsl:attribute name="onclick">
                                            <xsl:choose>
                                                <xsl:when test="(ImDocLoc='SERVER') and ($useCloudImage='YES')">javascript:downloadCloudFile('<xsl:value-of select="$jinglePath" />')</xsl:when>
                        ...
                        <xsl:when test="video !=''">
                         <xsl:variable name="cassette">
                          <xsl:choose>
                            ...
                            <!-- Populate URL to cloud location if useCloudImage is set and server option enabled in im image type  -->
                            <a>
                                <xsl:choose>
                                    <xsl:when test="$showStatusInfo='false'">
                                        <xsl:attribute name="onclick">
                                            <xsl:choose>
                                                <xsl:when test="(ImDocLoc='SERVER') and ($useCloudImage='YES')">javascript:downloadCloudFile('<xsl:value-of select="$cassette" />')</xsl:when>
                        ...
                        <xsl:when test="(position() = 1) and ( $dropfield != '')  and ($headerrow = 'false')">
                            <a>
                                ...
                                <xsl:choose>
                                    ...
                                    <xsl:otherwise>
                                        <xsl:attribute name="href">javascript:pick('<xsl:value-of select="$dropId" />', '<xsl:copy-of select="$dropfield" />', '<xsl:value-of select="$dropEnri" />')</xsl:attribute>
                                    </xsl:otherwise>
                                </xsl:choose>
                        ...
                        <xsl:when test="document !=''">
                            ...
                            <!-- Populate URL to cloud location if useCloudImage is set and server option enabled in cloud   -->
                            <a>
                                <xsl:choose>
                                    <xsl:when test="$showStatusInfo='false'">
                                        <xsl:attribute name="onclick">
                                            <xsl:choose>
                                                <xsl:when test="(ImDocLoc='SERVER') and ($useCloudImage='YES')">javascript:downloadCloudFile('<xsl:value-of select="$docPath" />')</xsl:when>
                                                              ...
                                                              <xsl:when test="(../c[position()]/drillInfo != '')">
                                                                 <xsl:if test="($drillInfo!='NO')">
                                                                    <xsl:choose>
                                                                        ...
                                                                        <xsl:otherwise>
                                                                            <xsl:attribute name="href">javascript:drilldown('<xsl:value-of select="/responseDetails/window/panes/pane/dataSection/enqResponse/drills/d[position()=$drillInfo]/item" />','<xsl:value-of select="../ref" />')</xsl:attribute>
                                                                        </xsl:otherwise>
                                                                    </xsl:choose>
                                                               ...
                                                               <xsl:otherwise>
                                                                    <xsl:choose>
                                                                        ...
                                                                        <xsl:otherwise>
                                                                            <xsl:attribute name="href">javascript:drilldown('<xsl:value-of select="/responseDetails/window/panes/pane/dataSection/enqResponse/drills/d[drillCol=$position]/item"/>','<xsl:value-of select="../ref"/>')</xsl:attribute>
                                                                        </xsl:otherwise>
                                                                    </xsl:choose>
                                                         ...
                                                         <xsl:when test="(../c[position()]/drillInfo != '')">
                                                            <xsl:if test="($drillInfo!='NO')">
                                                                <xsl:choose>
                                                                    ...
                                                                    <xsl:otherwise>
                                                                        <xsl:attribute name="href">javascript:drilldown('<xsl:value-of select="/responseDetails/window/panes/pane/dataSection/enqResponse/drills/d[$drillInfo]/item" />','<xsl:value-of select="../ref" />')</xsl:attribute>
                                                                    </xsl:otherwise>
                                                                </xsl:choose>
                                            ...
                                            <xsl:when test="img!=''">
                                                <xsl:variable name="imgPath">
                                                        <xsl:choose>
                                                            ...
                                                            <xsl:when test="(substring(img, 1, 3) = '../')">../servlet/RenderImageServlet?filePath=<xsl:value-of select="img"/>&amp;enqid=<xsl:value-of select="$enquiryId" />&amp;formToken=<xsl:value-of select="$csrfToken"/></xsl:when>
                                                            ...
                                                            <xsl:when test="(substring(img, 2, 1) = ':')">../servlet/RenderImageServlet?filePath=<xsl:value-of select="img"/>&amp;enqid=<xsl:value-of select="$enquiryId" />&amp;formToken=<xsl:value-of select="$csrfToken"/></xsl:when>
                                                            <xsl:when test="(substring(img, 1, 1) = '/') and not((ImDocLoc='SERVER') and ($useCloudImage='YES'))">../servlet/RenderImageServlet?filePath=<xsl:value-of select="img"/>&amp;enqid=<xsl:value-of select="$enquiryId" />&amp;formToken=<xsl:value-of select="$csrfToken"/></xsl:when>
                                                ...
                                                <xsl:choose>
                                                    <xsl:when test="(ImDocLoc='SERVER') and not($useCloudImage='YES')">
                                                        <img>
                                                           ...
                                                           <xsl:attribute name="src">../servlet/DisplayServerImage?docDownloadPath=<xsl:value-of select="img"/>&amp;downloadLocation=<xsl:value-of select="ImDocLoc" />&amp;enqid=<xsl:value-of select="$enquiryId" />&amp;formToken=<xsl:value-of select="$csrfToken"/></xsl:attribute>
                                                       </img>
                                                    </xsl:when>
                                                    <xsl:when test="(ImDocLoc!='') and ($docPath!='')">
                                                    <!-- Populate URL to cloud location if useCloudImage is set and server option enabled in im image type  -->
                                                    <a>
                                                        <xsl:choose>
                                                            <xsl:when test="$showStatusInfo='false'">
                        ...
                        <xsl:when test="img != ''">
                                ...
                                <img onmousedown = "return false;" onmouseup= "return false;" onclick= "return false;" oncontextmenu="return false;" onerror="this.src='../plaf/images/default/imagenotfound.jpg'">
                                    <xsl:attribute name="src">
                                        <xsl:choose>
                                            <xsl:when test="(ImDocLoc='SERVER') and ($useCloudImage='YES')">../GetImage?imageId=<xsl:value-of select="$imgPath"/>&amp;enqid=<xsl:value-of select="$enquiryId" />&amp;formToken=<xsl:value-of select="$csrfToken"/></xsl:when>
                            ...
                            <td>
                                ...
                                <table cellSpacing="0" cellPadding="0" border="0" summary="">
                                    ...
                                    <xsl:attribute name="class">
                                        ...
                                        <td width="1">
                                            <a class="iconLink" title="Select Drilldown" onfocus ="focusonKey('inline',event);" onblur = "hideTooltip(event);">
                                                <xsl:choose>
                                                    <xsl:when test="$showStatusInfo='false'">
                                                        <xsl:attribute name="onclick">javascript:drilldownSelection('drillbox:<xsl:value-of select="$myref"/>')</xsl:attribute>
                                                        <xsl:attribute name="href">javascript:void(0)</xsl:attribute>
                                                    </xsl:when>
                                                    <xsl:otherwise>
                                                        <xsl:attribute name="href">javascript:drilldownSelection('drillbox:<xsl:value-of select="$myref"/>')</xsl:attribute>
                                                    </xsl:otherwise>
                                                </xsl:choose>
                            ...
                            <xsl:if test="(/responseDetails/window/panes/pane/dataSection/enqResponse/fastPath ='NO')">
                                <td>
                                    ...
                                    <xsl:choose>
                                        ...
                                        <xsl:when test="img!=''">
                                            <a title="{cap}" onfocus ="focusonKey('inline',event);" onblur = "hideTooltip(event);">
                                                <xsl:choose>
                                                    <xsl:when test="$showStatusInfo='false'">
                                                        <xsl:attribute name="onclick">javascript:drilldown('<xsl:value-of select="item"/>','<xsl:value-of select="$myref"/>')</xsl:attribute>
                                                        <xsl:attribute name="href">javascript:void(0)</xsl:attribute>
                                                    </xsl:when>
                                                    <xsl:otherwise>
                                                        <xsl:attribute name="href">javascript:drilldown('<xsl:value-of select="item"/>','<xsl:value-of select="$myref"/>')</xsl:attribute>
                                                    </xsl:otherwise>
                                                </xsl:choose>
                                        ...
                                        <xsl:when test="drillCol!=''">
                                            ...
                                            <a class="enqDrillLink" tabindex="0">
                                                <xsl:choose>
                                                    <xsl:when test="$showStatusInfo='false'">
                                                        <xsl:attribute name="onclick">javascript:drilldown('<xsl:value-of select="item"/>','<xsl:value-of select="$myref"/>')</xsl:attribute>
                                                        <xsl:attribute name="href">javascript:void(0)</xsl:attribute>
                        ...
                        <table cellpadding="0" cellspacing="0" summary="" class="columnHeader">
                            <tr>
                                ...
                                <!-- see if the node has the attribute collapse attached -->
                                    ...
                                    <a accesskey="" title="{.}" tabindex="0" onfocus="focusonKey('inline',event);" onblur="hideTooltip(event);">
                                        <xsl:choose>
                                            <xsl:when test="$showStatusInfo='false'">
                                                <xsl:attribute name="onclick">javascript:switchColumn(<xsl:value-of select="position()"/>)</xsl:attribute>
                                                <xsl:attribute name="href">javascript:void(0)</xsl:attribute>
                                            </xsl:when>
                                            <xsl:otherwise>
                                                <xsl:attribute name="href">javascript:switchColumn(<xsl:value-of select="position()"/>)</xsl:attribute>
                                            </xsl:otherwise>
                                        </xsl:choose>
    ...
    <!-- Template for appending Right-to-left mark to words that have RTL direction for eg. Arabic. 
         Its achieved by following below steps,
         1. Form the new variable($new-rept) that contains words delimited by space. 
         2. Using the space as delimiter extract the word and spaces(includes new line, tabs etc.).
         3. Check if the word is arabic. If true concat &rlm; mark.
         4. Output the final word and spaces present before the delimiter.
         5. Check if any words are left after the delimiter($remaining) then call the template again.
            with remaining words present after the delimiter. -->
    <xsl:template name="concat-rlm-mark">
        <xsl:param name="report" />

        <xsl:variable name="new-rept" select="concat(normalize-space($report), ' ')" />
        <xsl:variable name="word" select="substring-before($new-rept, ' ')" />
        <xsl:variable name="spaces" select="substring-before($report, $word)" />
        <xsl:variable name="remaining" select="substring-after($new-rept, ' ')" />

        <xsl:choose>
            <!-- Test to check if any RTL chars are present. -->
            <xsl:when test="string-length(translate($word, $replaceChars, '')) &gt; 0">
                <xsl:variable name="new-word" select="concat($word, '&#8207;')" />
                <xsl:value-of select="concat($spaces, $new-word)" />
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="concat($spaces, $word)" />
            </xsl:otherwise>
        </xsl:choose>

        <xsl:choose>
            <xsl:when test="$remaining = ''">
                <!-- There might be spaces present after the last word. So below line preserves that space. -->
                <xsl:value-of select="substring-after($report, $word)" />
            </xsl:when>
            <xsl:otherwise>
                <xsl:call-template name="concat-rlm-mark">
                    <xsl:with-param name="report" select="substring-after($report, $word)" />
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
