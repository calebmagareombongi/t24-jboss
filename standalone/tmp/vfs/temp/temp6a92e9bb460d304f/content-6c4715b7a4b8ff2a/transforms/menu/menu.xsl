									...
									<a>
										...
										<xsl:choose>
											<xsl:when test="$showStatusInfo='false'">
												<xsl:attribute name="onclick">javascript:TabbedMenu.showTab(<xsl:value-of select="position() - 1"/>)</xsl:attribute>
												<xsl:attribute name="href">javascript:void(0)</xsl:attribute>
											</xsl:when>
											<xsl:otherwise>
												<xsl:attribute name="href">javascript:TabbedMenu.showTab(<xsl:value-of select="position() - 1"/>)</xsl:attribute>
											</xsl:otherwise>
										</xsl:choose>
