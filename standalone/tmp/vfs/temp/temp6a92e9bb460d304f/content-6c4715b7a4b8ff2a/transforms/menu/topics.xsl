		...
		<!-- Errors -->
										...
										<xsl:when test="errorhref!=''">
											<a tabindex="0">
												...
												<xsl:choose>
													...
													<xsl:otherwise>
														<xsl:attribute name="href"><xsl:value-of select="errorhref" /></xsl:attribute>
													</xsl:otherwise>
												</xsl:choose>
												<span>
													<xsl:choose>
					...
					<xsl:for-each select="warnings/warning">
						...
						<xsl:variable name="warningtext">
                            <xsl:call-template name="encodeAndEscape-quote">
                                <xsl:with-param name="string" select="warningtext"/>
                            </xsl:call-template>						
						</xsl:variable>
			...
			<xsl:if test="$isConfirm !='YES'">
				<table cellpadding="0" cellspacing="0">
					<tr>
						<td class="textbtn">
							<a tabindex="0" id="errorImg">
								<xsl:choose>
									<xsl:when test="$showStatusInfo='false'">
										<xsl:attribute name="onclick">javascript:commitOverrides()</xsl:attribute>
										<xsl:attribute name="href">javascript:void(0)</xsl:attribute>
									</xsl:when>
									<xsl:otherwise>
										<xsl:attribute name="href">javascript:commitOverrides()</xsl:attribute>
									</xsl:otherwise>
								</xsl:choose>
