	...
	<xsl:template match="tool" name="tool_n">
		...
		<xsl:variable name="enquiryFind">
			...
			<xsl:choose>
				...
				<xsl:when test="ty='REQUEST'">../jsps/genrequest.jsp?&amp;routineName=<xsl:value-of select="routine"/>&amp;routineArgs=<xsl:value-of select="args"/>&amp;skin=<xsl:copy-of select="$skin"/>&amp;compId=<xsl:copy-of select="$atoolcompid"/>&amp;compScreen=<xsl:copy-of select="$compScreen"/>&amp;contextRoot=<xsl:copy-of select="$contextRoot"/>&amp;user=<xsl:copy-of select="$user"/>&amp;formToken=<xsl:value-of select="$csrfToken"/></xsl:when>
						   ...
						   <a>
								...
								<xsl:choose>
									<xsl:when test="$showStatusInfo='false'">
										<xsl:attribute name="onclick">javascript:Identifycombovalue('COMBO_<xsl:value-of select="n"/>','<xsl:value-of select="$routine" />')</xsl:attribute>
										<xsl:attribute name="href">javascript:void(0)</xsl:attribute>
									</xsl:when>
									<xsl:otherwise>
										<xsl:attribute name="href">javascript:Identifycombovalue('COMBO_<xsl:value-of select="n"/>','<xsl:value-of select="$routine" />')</xsl:attribute>
									</xsl:otherwise>
								</xsl:choose>
