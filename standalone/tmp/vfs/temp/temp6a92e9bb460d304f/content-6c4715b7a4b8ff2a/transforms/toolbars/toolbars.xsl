...
<xsl:import href="tool.xsl"/>
	<xsl:template match="toolbars" name="toolbars_n">
		...
		<xsl:variable name="toolBarDisType"><xsl:value-of select="toolbarDisType"/></xsl:variable>
		<xsl:variable name="toolBarId"><xsl:value-of select="toolbarId"/></xsl:variable>
		<xsl:variable name="toolBarType" select="string(/responseDetails/window/panes/pane/contract/toolbartype)"/>
			<xsl:choose>
				<!-- If Orientation is set for VERTICAL display -->
				<xsl:when test="$toolBarDisType='VERTICAL'">
					<xsl:variable name="toolBarStyle"><xsl:value-of select="toolbarStyle"/></xsl:variable>
						<div id="{$toolBarStyle}">
							<xsl:if test="toolbarText!=''">
								<p class="message">
									<xsl:value-of select="toolbarText"/>
								</p>
							</xsl:if>
							<table class="{$toolBarStyle}">
								<!-- Tools -->
								<xsl:for-each select="tool">
									<xsl:if test="$isArc='false' and ($toolBarId='APPLICATION' or $toolBarId='CONTRACT' or $toolBarType='VERSION.TOOLBAR') and position()=last()">
