                        ...
                        <xsl:when test="routine!=''">
                            <xsl:choose>
                                <xsl:when test="routine='OS.GET.COMPOSITE.SCREEN.XML'">
                                    <xsl:attribute name="src">../jsps/genrequest.jsp?&amp;routineName=<xsl:value-of select="routine"/>&amp;routineArgs=<xsl:value-of select="$args"/>&amp;skin=<xsl:value-of select="$skin"/>&amp;compId=<xsl:value-of select="$companyid"/>&amp;usrRole=<xsl:value-of select="$userrole"/>&amp;compScreen=<xsl:value-of select="$compScreen"/>&amp;contextRoot=<xsl:value-of select="$contextRoot"/>&amp;windowName=<xsl:value-of select="n"/>&amp;user=<xsl:value-of select="$user"/>&amp;WS_replaceAll=<xsl:value-of select="replaceAll"/>&amp;WS_parentComposite=<xsl:value-of select="$id"/>&amp;formToken=<xsl:value-of select="$csrfToken"/></xsl:attribute>
                                </xsl:when>
                                <xsl:otherwise>
