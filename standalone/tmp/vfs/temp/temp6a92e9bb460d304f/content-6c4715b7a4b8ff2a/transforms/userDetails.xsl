...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="userDetails">
		...
		<!-- If user wants to open document service pdf files in Browser itself add this flag to let the client know.  -->
		<xsl:if test="/responseDetails/userDetails/openDocDownloadContentInInternetBrowser">
			<input type="hidden" name="openDocDownloadContentInInternetBrowser" id="openDocDownloadContentInInternetBrowser" >
				<xsl:attribute name="value"><xsl:value-of select="/responseDetails/userDetails/openDocDownloadContentInInternetBrowser" /></xsl:attribute>
			</input>
		</xsl:if>		
		
		<input type="hidden" name="noRightClick" id="noRightClick">
            <xsl:attribute name="value"><xsl:value-of select="/responseDetails/NoRightClick"/></xsl:attribute>
        </input>
