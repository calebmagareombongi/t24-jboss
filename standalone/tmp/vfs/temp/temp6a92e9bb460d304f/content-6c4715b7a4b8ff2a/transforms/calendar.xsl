          ...
          <td>
		   <select>
				...
				<xsl:if test="$popupDropDown='true'">
					...
					<xsl:attribute name="onmousedown">calendarDateMouseOver()</xsl:attribute>
			...
			<xsl:choose>
				<xsl:when test="$showStatusInfo='false'">
					<xsl:attribute name="onclick">
						<xsl:choose>
							<xsl:when test="$popupDropDown='true'">javascript:dropCalendarPickDate('<xsl:value-of select="." />', '<xsl:value-of select="/responseDetails/cal/month" />', '<xsl:value-of select="/responseDetails/cal/year" />', '<xsl:value-of select="/responseDetails/cal/df" />','<xsl:value-of select="/responseDetails/webDetails/WS_FragmentName" />')</xsl:when>
							<xsl:otherwise>javascript:pickDate('<xsl:value-of select="." />', '<xsl:value-of select="/responseDetails/cal/month" />', '<xsl:value-of select="/responseDetails/cal/year" />', '<xsl:value-of select="/responseDetails/cal/df" />')</xsl:otherwise>
						</xsl:choose>
					</xsl:attribute>
