		...
		<html>
			...
			<form name="attachment" method="POST" ENCTYPE="multipart/form-data" action="UploadServlet">
					...
					<xsl:for-each select="responseDetails/att/mainbody">
						...
						<input type="hidden" name="uploadField">
							<xsl:attribute name="value"><xsl:value-of select="df"/></xsl:attribute>
						</input>
						<input type="hidden" name="imageId">
							<xsl:attribute name="value"><xsl:value-of select="imageid"/></xsl:attribute>
						</input>
						<input type="hidden" name="imgMaxFileSize">
							<xsl:attribute name="value"><xsl:value-of select="imgMaxFileSize"/></xsl:attribute>
						</input>
												...
												<p align="right">
													<a>
														...
														<xsl:choose>
															...
															<xsl:otherwise>
																<xsl:attribute name="href">javascript:fileUpload('<xsl:value-of select="df" />')</xsl:attribute>
															</xsl:otherwise>
														</xsl:choose>
														<img border="0">
