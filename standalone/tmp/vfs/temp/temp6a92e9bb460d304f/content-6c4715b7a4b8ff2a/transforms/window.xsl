			...
			<head>
				...
				<xsl:choose>
					<xsl:when test="responseDetails/window/frames!=''">
                  		<script type="text/javascript">noFrames = <xsl:value-of select="$noFrames"/>; top.noFrames = <xsl:value-of select="$noFrames"/>; top.keepaliveHandling = <xsl:value-of select="$keepaliveHandling"/>;top.useconfirm=<xsl:value-of select="$useconfirm"/>;top.autoHoldDeals = <xsl:value-of select="$autoHoldDeals"/>;</script>
               		</xsl:when>
                	<xsl:otherwise><script type="text/javascript">noFrames = <xsl:value-of select="$noFrames"/>; top.noFrames = <xsl:value-of select="$noFrames"/>;</script></xsl:otherwise>
                </xsl:choose>				
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:element name="body">
						...
						<!-- check if we are processing enquiry wihin a frame, then insert value of $noFrames to get exact value for top.noFrames. -->
						<xsl:if test="((responseDetails/window/panes/pane/dataSection/enqResponse!='') and ($noFrames = 'false'))">						
						        <input type="hidden" name="topNoFrames" id="topNoFrames">
                                      <xsl:attribute name="value"><xsl:copy-of select = "$noFrames" /></xsl:attribute>
                                </input>
               		    </xsl:if>
