...
<%
    String realfname = request.getParameter("actualfname");
    ...
    String noRightClick = request.getParameter("noRightClick");
%>
...
<body dir= <%= lngDir %> onload="runContext( '<%= ceids %>','<%= enqids %>', '<%= enqdescs %>','<%= skin %>', '<%= realfname %>','<%= olddata %>','<%= fname %>','<%= formId %>')" oncontextmenu='<%= noRightClick %>' >
